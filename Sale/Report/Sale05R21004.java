...
public class Sale05R21004 extends bTransaction {
  ...
  public void doExcel() throws Throwable {
    ...
    Vector retVector = exeExcel.getExcelObject("G:\\資訊室\\Excel\\Sale\\Sale05R21004.xlt");
    ...
    for (int intP = 0; intP < retProjectID1.length; intP++) {
      ...
      // **跑各棟樓別資料 (主要列)
      retPositionCarGifts = getPositionCarGifts(stringProjectID1, stringOrderDateS, stringOrderDateE, stringIsExportGifts, conDateS, conDateE);
  ...
  public String[][] getPositionCarGifts(String stringProjectID1, String stringOrderDateS, String stringOrderDateE, String stringIsExportGifts, String conDateS , String conDateE) throws Throwable {
    ...
    if ("Yes".equals(stringIsExportGifts)) {
      ...
      stringSql += "'' ItemNo, '' Qty, '' TotalAmt ";
    }
    stringSql += "from Sale05M090 T90, Sale05M040 T40, Sale05M092 T92 ";
    if ("Yes".equals(stringIsExportGifts)) {
    ...
    if (stringOrderDateS.length() > 0) {
      stringSql += "and T90.OrderDate between '" + stringOrderDateS + "' and '" + stringOrderDateE + "' ";
    }
    
    //簽約日條件
    if (conDateS.length() > 0) {
      stringSql += "and T90.OrderNo in (select b.OrderNo from sale05m274 a , Sale05M275_New b where a.ContractNo = b.ContractNo and a.IsVoid = 'N' "
                 + "and a.ContractDate between '"+conDateS+"' and '"+conDateE+"') ";
    }
    
    stringSql += "order by T90.OrderNo, T92.HouseCar desc, T92.Position";
    retData = dbSale.queryFromPool(stringSql);
    //
    return retData;
  }
