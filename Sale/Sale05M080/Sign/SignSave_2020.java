...
public class SignSave_2020 extends bproc {
  ...
  public String getDefaultValue(String value) throws Throwable {
    ...
    //
    // doInsert_Sale05M080_Flow_HIS( ) ; // 歷史檔記錄(可多筆) Primary Key 為
    // DocNo、FlowStatus、EDateTime。
    // doInsert_Sale05M080_Flow( ) ; // 歷史檔記錄 Primary Key 為
    // DocNo、FlowStatus、EDateTime。
    ...
    if (stringFlowStatus.equals("業管-審核")) {
      ...
      for (int intCompany = 0; intCompany < vectorCompanyCd.size(); intCompany++) {
        ...
        // System.out.println(intCompany + "(保護發票控管檔)--------------------------------" +
        // stringCompanyCd) ;
...
// 由 Sale05M084 取得之所有客戶 START
      ...
      // System.out.println("(LOOP1)由 Sale05M084
      // 取得之所有客戶--------"+retSale05M084P.length) ;
...
// 取得特定客戶的購屋證明單 START                           
        ...
        for (int intSale05M086 = 0; intSale05M086 < retSale05M086.length; intSale05M086++) {
          ...
          // 4 H_MomentaryMoney 5 H_UsableMoney 6 H_ReceiveMoney 7 (H_ReceiveMoney +
          // H_UsableMoney) AS H_InvoiceMoney
          // 8 L_MomentaryMoney 9 L_UsableMoney 10 L_ReceiveMoney 11 (L_ReceiveMoney +
          ...
          for (int intSale05M081 = 0; intSale05M081 < retSale05M081.length; intSale05M081++) {
            ...
            for (int intInvoice = 1; intInvoice <= 2; intInvoice++) {
              ...
              /*
               * System.out.println(stringCustomNo + "-----------------" + stringCompanyCd +
               * "-----------------" + stringPosition + "-----------------" +
               * arrayPointNo[intType] + "-----------------" + intCount + "-----------------"
               * + stringH_DiscountMoneyUse) ;
               */
              // 0 stringInvoiceKind 1 stringInvoiceTotalMoney 2 stringITEMLS_CD 3
              // stringORDER_NO 4 stringPosition
            ...
            for (int intInvoice = 1; intInvoice <= 2; intInvoice++) {
               ...
               * System.out.println(stringCustomNo + "-----------------" + stringCompanyCd +
               * "-----------------" + stringPosition + "-----------------" +
               * arrayPointNo[intType-4] + "-----------------" + intCount +
               * "-----------------" + stringFeeMoneyUse + "-----------------" +
               * stringH_DiscountMoneyUse) ;
               */
              // 0 stringInvoiceKind 1 stringInvoiceTotalMoney 2 stringITEMLS_CD 3
            ...
            if (vectorSale05M061.indexOf(stringSUMReceiveCheck) == -1) {
              vectorSale05M061Sql = doUpdateSale05M061(stringH_MomentaryMoney, stringL_MomentaryMoney, stringProjectID1, stringHouseCar, stringPosition, stringORDER_NO,
                  vectorSale05M061Sql);
...
// 由 Sale05M084 取得之所有客戶  END
      ...
      for (int intCompany = 0; intCompany < vectorCompanyCd.size(); intCompany++) {
        ...
        for (int intNo = 0; intNo < vectorInvoiceKind.size(); intNo++) {
          ...
          // System.out.println(intCompany + "-----------" +
          // intNo+"-----------"+intAvailableInvoice+"------------------"+intInvoiceTotalCount)
          // ;
          if (intInvoiceTotalCount > intAvailableInvoice) {
            // message("公司：" + getCompanyName(stringCompanyCd) + "之電腦發票不足！ 請洽 財務室 領取！尚缺" +
            // (intInvoiceTotalCount - intAvailableInvoice) + "張") ;
            message("公司：" + getCompanyName(stringCompanyCd) + "、案別：" + stringProjectID1 + " 之 " + stringInvoiceKind + " 聯式電腦發票不足！ 請洽 財務室 領取！尚缺"
                + (intInvoiceTotalCount - intAvailableInvoice) + "張");
          ...
          for (int intCompanNo = 0; intCompanNo < vectorCompanyCd.size(); intCompanNo++) {
            ...
            for (int intNo = 0; intNo < 4; intNo++) {
              ...
              for (int intRecord = 1; intRecord <= intRecordMax; intRecord++) {
                ...
                /*
                 * System.out.println("後--------------------"+ stringCustomNo +
                 * "-----------------" + stringCompanyCd + "-----------------" + stringPosition
                 * + "-----------------" + arrayPointNo[intNo] + "-----------------" + intRecord
                ...
                if (intRecord % 5 == 1) {
                  ...
                  // 發票已開完Flag
                  if (stringInvoiceNo.equals(stringInvoiceEndNo)) stringEndYes = "Y";
                  else
                    stringEndYes = "N";
                  // 發票訊息
                ...
                // 計算稅率
                /*
                 * // 0 TaxRate 1 TaxKind retInvoM010 = getInvoM010(arrayPointNo[intNo]) ; if
                 * (retInvoM010.length == 0){ message("發票系統.摘要代碼 錯誤!"); return value ; }
                 */
                // 存入 DB
                /* 發票.InvoM030(Head)(一筆) */
                if (intNo == 0) {
                ...
                if (intRecord % 5 == 0 || !booleanNextNotNull) {
                  ...
                  /*
                   * if("H72A".equals(stringProjectID1) && booleanFlag &&
                   * "2103".equals(stringPointNo) && !"A30148".equals(stringPosition)) {
                   * //if("H72A".equals(stringProjectID1) && "01002".equals(stringITEMLS_CD) &&
                   * "2103".equals(stringPointNo) && !"A30148".equals(stringPosition)) {
                   * stringPointNo = "2113" ; } if("H72A".equals(stringProjectID1) && booleanFlag
                   * && "2104".equals(stringPointNo) && !"A30148".equals(stringPosition)) {
                   * //if("H72A".equals(stringProjectID1) && "01002".equals(stringITEMLS_CD) &&
                   * "2104".equals(stringPointNo) && !"A30148".equals(stringPosition)) {
                   * stringPointNo = "2112" ; }
                   */
                  ...
                  // 2101-3-5 修改 End
                  doInvertInvoM030(stringInvoiceNo, stringInvoiceKind, stringPosition, stringCustomNo, stringPointNo, convert.FourToFive("" + doubleInvoiceMoney, 0),
                      convert.FourToFive("" + doubleInvoiceTax, 0), convert.FourToFive("" + doubleInvoiceTotalMoney, 0), stringTaxKind, stringCompanyCd, retSale05M080,
                      "" + doubleDiscountMoney, intDiscountCountInvoice);
                  doubleInvoiceMoney = 0;
                  doubleInvoiceTotalMoney = 0;
                  doubleInvoiceTax = 0;
                  intDiscountCountInvoice = 0;
                ...
                if (intRecord % 5 == 1) {
                  doUpdateSale05M085(stringInvoiceNo, stringEDate, stringEndYes, stringCompanyCd, stringInvoiceYYYYMM, stringFSChar, stringStartNo, stringInvoiceBook);
                }
                //
                if (intNo != 0) continue;
                else {
                  // System.out.println(intSale05M084P+"-------" + intSale05M081+"-------"
                  // +intNo+"--------------------------------------------"+intRecord+"--------A")
                  // ;
                  if (!("".equals(stringL_DiscountMoney) || doParseDouble(stringL_DiscountMoney) == 0)) {
                    // System.out.println(intSale05M084P+"-------" + intSale05M081+"-------"
                    ...
                    doInsertInvoM041(stringDiscountNo, intDiscountCount, stringInvoiceNo, arrayPointNo[intNo], "" + (doParseDouble(stringL_DiscountMoney) * 10000));
                    //
            ...
            for (int intNo = 4; intNo < 8; intNo++) {
              ...
              for (int intRecord = 1; intRecord <= intRecordMax; intRecord++) {
                ...
                stringLCom = (String) vectorReturn.get(7);
                /*
                 * System.out.println("後-消費券--------------------"+ stringCustomNo +
                 * "-----------------" + stringCompanyCd + "-----------------" + stringPosition
                 * + "-----------------" + arrayPointNo[intNo-4] + "-----------------" +
                 * intRecord + "------------------" + stringInvoiceTotalMoney +
                 * "------------------" + stringL_DiscountMoney) ;
                 */
                // 判斷下一筆是否有值
                ...
                if (intRecord % 5 == 0 || !booleanNextNotNull) {
                  ...
                  doInvertInvoM030(stringInvoiceNo, stringInvoiceKind, stringPosition, stringCustomNo, stringPointNo, convert.FourToFive("" + doubleInvoiceMoney, 0),
                      convert.FourToFive("" + doubleInvoiceTax, 0), convert.FourToFive("" + doubleInvoiceTotalMoney, 0), stringTaxKind, stringCompanyCd, retSale05M080, "0", 0);
                  doubleInvoiceMoney = 0;
                  doubleInvoiceTotalMoney = 0;
                  doubleInvoiceTax = 0;
                  intDiscountCountInvoice = 0;
      ...
      if (vectorSale05M061Sql.size() > 0) dbSale.execFromPool((String[]) vectorSale05M061Sql.toArray(new String[0]));
      //
      for (int intCompanNo = 0; intCompanNo < vectorCompanyCd.size(); intCompanNo++) {
        stringCompanyCd = "" + (String) vectorCompanyCd.get(intCompanNo);
  ...
  public void doInvoM022Undo(Vector vectorCompanyCd, int intCompanySize) throws Throwable {
    ...
    // System.out.println("回復 InvoM022 之狀態
    // START------------------------------------------------------------"+intCompanySize)
    // ;
  ...
  public void doCount(int intPos, int intType, String stringCustomNo, String stringInvoiceKind, String[][] retSale05M040, Hashtable hashtableCustomCompanyPositionTypeCount,
      Hashtable hashtableInvoiceTotalCount) throws Throwable {
    int intCount = 0;
  ...
  public Vector getCompanys(String stringDocNo, String stringProjectID1) throws Throwable {
    ...
    for (int intSale05M040 = 0; intSale05M040 < retSale05M040.length; intSale05M040++) {
      ...
      // if("".equals(stringHCompany) || "".equals(stringLCompany)) stringStatus =
      // "NULL" ;
  ...
  public void doInsertInvoM041(String stringDiscountNo, int intDiscountCount, String stringInvoiceNo, String stringITEMLS_CHINESE, String stringInvoiceTotalMoney)
      throws Throwable {
    stringInvoiceTotalMoney = convert.FourToFive("" + stringInvoiceTotalMoney, 0);
  ...
  public void doUpdateSale05M061(int intInvoice, String stringReceiveMoney, String stringProjectID1, String stringHouseCar, String stringPosition, String stringORDER_NO)
      throws Throwable {
  ...
  public String[][] getSale05M080Tmp() throws Throwable {
    ...
    String[][] retSale05M080 = null;
    //
    stringSql = " SELECT  CompanyNo,  DepartNo,  ProjectID1,  EDate " + " FROM  Sale05M080 " + " WHERE  DocNo  =  '" + getValue("DocNo").trim() + "' ";
  ...
  public String[][] getSale05M081ForCustomNo(String stringDocNo, String stringCustomNo) throws Throwable {
    ...
    // 5 H_MomentaryMoney 6 H_UsableMoney 7 H_ReceiveMoney 8 (H_ReceiveMoney +
    // H_UsableMoney) AS H_InvoiceMoney
  ...
  public void doInvertInvoM030(String stringInvoiceNo, String stringInvoiceKind, String stringPosition, String stringCustomNo, String stringPointNo, String stringInvoiceMoney,
    ...
    // String stringCompanyNo = retSale05M080.length!=0 ?
    // retSale05M080[retSale05M080.length-1][0].trim( ) : "" ;
    String stringDepartNo = retSale05M080.length != 0 ? retSale05M080[retSale05M080.length - 1][1].trim() : "";
    String stringProjectID1 = retSale05M080.length != 0 ? retSale05M080[retSale05M080.length - 1][2].trim() : "";
    String stringEDate = retSale05M080.length != 0 ? retSale05M080[retSale05M080.length - 1][3].trim() : "";
    ...
    String stringSql = " INSERT  INTO  InvoM030 (InvoiceNo,                InvoiceDate,  InvoiceKind,              CompanyNo,  DepartNo, "
        ...
        + "', " + " '" + stringEDate + "', " + " '" + stringInvoiceKind + "', " + " '" + stringCompanyCd + "', " + " '" + stringDepartNo + "', " + // 04
        ...
        + stringUserID + "', " + " '" + stringDateTime + "', " + " '" + r1.nextInt(9999) + "', " + " '" + customName + "', " + "  ) ";
