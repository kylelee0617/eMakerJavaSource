...
public class CancelDiscount extends bproc {
  ...
  public String getDefaultValue(String value) throws Throwable {
     ...
     * exeFun.setPreView(false, "D:\\oce.xls") ; // 當不預覽時，且有傳入路徑時，另存新檔。 int
     * intRecord = 0 ; // 測試 END
     */
    // 收款單明細筆數
    ...
    for (int intSale05M081 = 0; intSale05M081 < retSale05M081.length; intSale05M081++) {
      ...
      if (booleanFlag) {
        /*
         * 測試用 START exeFun.putDataIntoExcel(0, intRecord, "開立折讓頭檔資料進去 DB 中(每個顧客只做一次)",
         * objectSheet1) ; intRecord++ ; 測試用 END
         */
        intRecordNo = 1;
        // 取得折讓單號碼
        String stringTemp = "";
        ...
        /*
         * // 新增 stringSQL="INSERT INTO Invom040 " + "SELECT '" + stringNo + "','" +
         * stringEDate +"', CompanyNo," + "'" + stringDepartNo+ "',ProjectNo, ''," +
         * "CustomNo,'A', '' ,0,"+ "0 ,0, 'N' ,0 ,'N' ,"+ "'N' ,'"+ getUser() +"', '" +
         * datetime.getTime("YYYY/mm/dd h:m:s") +"', '1' ,'收款.退戶' " + " FROM  Invom030 "
        ...
        if (stringProjectID1.equals("H90A")) { // Mei1011023
          ...
          stringSQL = "INSERT INTO Invom040 " + "SELECT '" + stringNo + "','" + stringEDate + "', CompanyNo," + "'" + stringDepartNo + "','H90A', ''," + "CustomNo,'A', '' ,0,"
              + "0 ,0, 'N' ,0 ,'N' ," + "'N' ,'" + getUser() + "', '" + datetime.getTime("YYYY/mm/dd h:m:s") + "', '1' ,'收款.退戶' , '' , '' " + " FROM  Invom030 "
      ...
      /*
       * 測試用 START doShowDataInExcel(1, intRecord, objectSheet1, exeFun, arrayFiled,
       * retSale05M081[intSale05M081]) ; intRecord++ ; 測試用 END
       */
    ...
    /*
     * String OrderNo = getValue("field3").trim(); //使用編號 String TrxDate =
     * getValue("field2").trim(); //退戶日期 showForm("退戶-同步-行銷(Sale02M040)");
    ...
    for (int intSale05M092 = 0; intSale05M092 < retSale05M092.length; intSale05M092++) {
      ...
      if (Float.parseFloat(retA_Sale[0][43].trim()) > 0) {
        stringSQL = "INSERT INTO " + stringA_Sale + " ( " + " OperatorPosition," + " OperatorIndex," + " HouseCar," + " OperatorDom," + " YearMM," + " Com," + " Depart,"
            ...
            + " AO_sn," + " Plan1," + " ComNo," + " OrderNo" + " ,SSMediaID " + // 2010-12-03 SSMediaID 新增
      ...
      stringSQL = "UPDATE A_Sale " + " SET YearMM = ''," + " Depart = 0," + " Custom =''," + " OrderDate =''," + " OrderMon = 0," + " H_OrderMon = 0," + " L_OrderMon = 0,"
          ...
          + " AO_sn = 0," + " Plan1 =''," + " ComNo =''," + " OrderNo =''" + " , SSMediaID ='' " + // 2010-12-03 新增
...
//"退戶-同步-行銷(Sale02M040)"--------------------------------
    ...
    /*
     * 測試 START // 釋放 Excel 物件 exeFun.getReleaseExcelObject(retVector) ; // 測試 END
     */
