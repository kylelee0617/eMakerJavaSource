...
public class Sale05M27401_New extends bTransaction {
  ...
  public boolean chkM355() throws Throwable {
    String thisTable = "table27";
    JTable tb27 = this.getTable(thisTable);

    String lastCust_trust = "";
    for (int i = 0; i < tb27.getRowCount(); i++) {
      String custNo = getValueAt(thisTable, i, "CustomNo").toString().trim();
      String custName = getValueAt(thisTable, i, "CustomName").toString().trim();
      String trustNo = getValueAt(thisTable, i, "TrusteeId").toString().trim();
      String trustName = getValueAt(thisTable, i, "TrusteeName").toString().trim();
      if (StringUtils.isBlank(custNo) || StringUtils.isBlank(custName) || StringUtils.isBlank(trustNo) || StringUtils.isBlank(trustName)) {
        message("第" + (i + 1) + "列被委託人身分證字號、姓名、主客戶 不得空白。");
        return false;
      }

      if (StringUtils.equals(custNo + custName + trustNo + trustName, lastCust_trust)) {
        message("第" + (i + 1) + "列被委託人及主客戶 不得重複。");
        return false;
      }

      lastCust_trust = custNo + custName + trustNo + trustName;
    }
...
//            }
  ...
  public String getDescript(String stringContractNo, String stringCompanyCd) throws Throwable {
    ...
    //
    stringSql = "select Position " + "from Sale05M278 " + "where ContractNo='" + stringContractNo + "' " + "and CompanyCd='" + stringCompanyCd + "' " + // 修改日期:20120517 員工編號:B3774
