  ...
  public String getDefaultValue(String value) throws Throwable {
    ...
    if (retData.length > 0) {
      for (int a = 0; a < retData.length; a++) {
        ...
        // AML部分
        stringSql = "SELECT OrderDate,EDate,CDate,RecordDesc,Func,RecordType,ActionName,SHB98,SHB06B,customid, customName FROM Sale05M070 WHERE RecordDesc <> '不符合' AND RecordDesc<>'不適用' AND SHB06B <> '022' AND SHB06B <> '018'  AND orderNo='"
            + strOrderNo + "'  " + " UNION "
            + " SELECT OrderDate,EDate,CDate,RecordDesc,Func,RecordType,ActionName,SHB98,SHB06B,customid, customName FROM Sale05M070 WHERE RecordDesc <> '不符合' AND RecordDesc<>'不適用' AND SHB06B <> '022' AND SHB06B = '018'  AND RecordType = '附件-指定第三人'  AND orderNo='"
