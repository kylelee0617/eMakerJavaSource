...
public class Sale05R21004 extends bTransaction{
        ...
        for(int intP=0; intP<retProjectID1.length; intP++){
            ...
            for(int intNo=0; intNo<retPositionCarGifts.length; intNo++){
                ...
                if(intNo  !=  retPositionCarGifts.length - 1){
                    if(!(retPositionCarGifts[intNo+1][0].trim().equals(stringPosition) && 
                       retPositionCarGifts[intNo+1][1].trim().equals(stringCar))){
                        intInsertDataRow++;
                    }
                }else{
                    intInsertDataRow++;
            ...
            if("Yes".equals(stringIsExportGifts)){
                ...
                // 框線
                exeExcel.doLineStyle2("X"+(intDataEndRow+1)+":"+exeExcel.getExcelColumnName( "A",vectorItemNo.size()*2+22)+(intDataEndRow+1), objectSheet1,  "1", "4", "1", "3", "1", "4", "1", "3", "1", "2");
    ...
    public double doParseDouble(String stringAmt) throws Exception{
        double doubleAmt = 0;
        //
        if("".equals(stringAmt) || "null".equals(stringAmt)) return 0;
        try{
    ...
    public String getSpec(String stringItemNo) throws Throwable{
        ...
        //
                  // 0
        stringSql = "select Spec "+
                  "from Sale05M206 "+
                  "where ItemNo='"+stringItemNo+"'";
        retData = dbSale.queryFromPool(stringSql);
        if(retData.length > 0){
    ...
    public String[][] getPositionCarGifts(String stringProjectID1, String stringOrderDateS, String stringOrderDateE, 
                                String stringIsExportGifts) throws Throwable{
        String stringSql   = "";
        String retData[][] = null;
        //
                  // 0
        stringSql = "select (case when T92.HouseCar='House' then T92.Position else '' end) Position, "+
                  "(case when T92.HouseCar='Car' then T92.Position else '' end) Car, "+
                  "T90.OrderNo, "+
                  "T90.OrderDate, "+
                  "(select distinct (case when convert(char(10),ContrDate,111)='1900/01/01' then '' "+
                                   "else convert(char(10),ContrDate,111) end) "+
                   "from A_Sale "+
                   "where ProjectID1=T90.ProjectID1 "+
                   "and HouseCar=(case when T92.HouseCar='House' then 'Position' else 'Car' end) "+
                  ...
                  "T92.CommMoney, "+
                  // 10
                  "(T92.DealMoney -T92.GiftMoney -T92.CommMoney -T92.ViMoney) PureMoney, "+
                  "T40.FloorPrice, "+
                  "(T92.DealMoney -T92.GiftMoney -T92.CommMoney -T92.ViMoney -T40.FloorPrice) BalaMoney, "+
                  "T90.SaleName1, "+
                  "T90.SaleName2, "+
                  // 15
                  "T90.SaleName3, "+
        ...
        if("Yes".equals(stringIsExportGifts)){
            stringSql = stringSql + 
                      "T210.ItemNo, "+
                      "ISNULL(T210.Qty,0) Qty, "+
                      "ISNULL(T210.TotalAmt,0) TotalAmt ";
        }else{
            stringSql = stringSql + 
                      "'' ItemNo, "+
                      "'' Qty, "+
        ...
        stringSql = stringSql + 
                  "from Sale05M090 T90, Sale05M040 T40, Sale05M092 T92 ";
        if("Yes".equals(stringIsExportGifts)){
            stringSql = stringSql + 
                      "left join Sale05M210 T210 "+
                          "on T92.OrderNo=T210.OrderNo "+
                          "and T92.Position=T210.Position ";
        }
        stringSql = stringSql + 
                  "where T90.OrderNo=T92.OrderNo "+
                  "and T90.ProjectID1=T40.ProjectID1 "+
                  "and T92.HouseCar=T40.HouseCar "+
                  "and T92.Position=T40.Position "+
                  "and T90.ProjectID1='"+stringProjectID1+"' "+
        ...
        stringSql = stringSql + 
                  "order by T90.OrderNo, T92.HouseCar desc, T92.Position";
        retData = dbSale.queryFromPool(stringSql);
        //
        return  retData;
    }
    ...
    public String[][] getProjectID1(String stringProjectID1, String stringOrderDateS, String stringOrderDateE) throws Throwable{
        ...
        //
                  // 0
        stringSql = "select distinct T90.ProjectID1 "+
                  "from Sale05M090 T90, Sale05M092 T92 "+
                  "where T90.OrderNo=T92.OrderNo "+
                  "and ISNULL(T92.StatusCd,'')='' ";
        ...
        if(stringOrderDateS.length() > 0){
            stringSql = stringSql +
                      "and T90.OrderDate>='"+stringOrderDateS+"' ";
        }
        if(stringOrderDateE.length() > 0){
            stringSql = stringSql +
                      "and T90.OrderDate<='"+stringOrderDateE+"' ";
        }
        stringSql = stringSql +
                  "order by T90.ProjectID1";
        retData = dbSale.queryFromPool(stringSql);
        //
        return retData;
    }
    ...
    public String[][] getItemNo(String stringProjectID1, String stringOrderDateS, String stringOrderDateE) throws Throwable{
        ...
        //
                  // 0
        stringSql = "select distinct T210.ItemNo "+
                  "from Sale05M210 T210, Sale05M090 T90, Sale05M092 T92 "+
                  "where T210.ProjectID1='"+stringProjectID1+"' "+
                  "and T210.TotalAmt > 0 "+
                  "and T210.OrderNo=T90.OrderNo "+
                  "and T210.OrderNo=T92.OrderNo "+
