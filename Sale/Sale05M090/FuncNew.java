  ...
  public boolean action(String value) throws Throwable {
    ...
    for (int intRow = 0; intRow < jtableTable1.getRowCount(); intRow++) {
      ...
      if ("".equals(stringCountryName)) {
        messagebox("客戶第 " + (intRow + 1) + " 列之[國別] 必須輪入。");
        jtableTable1.setRowSelectionInterval(intRow, intRow);
        return false;
      }
      ...
      if (!"".equals(stringCellphone) && stringCellphone.length() != 10) {
        messagebox("客戶第 " + (intRow + 1) + " 列之[行動電話] 大小須為 10 碼。");
        jtableTable1.setRowSelectionInterval(intRow, intRow);
        return false;
      }
      ...
      if (retTown.length == 0) {
        messagebox("客戶第 " + (intRow + 1) + " 列之[縣市][鄉鎮] 關係不正確。");
        jtableTable1.setRowSelectionInterval(intRow, intRow);
        return false;
      }
      if (!stringZIP.equals(retTown[0][0].trim())) {
        ...
        if (!stringZIP.equals(retTown[0][0].trim())) {
          messagebox("客戶第 " + (intRow + 1) + " 列之[郵遞區號] 不正確。");
          jtableTable1.setRowSelectionInterval(intRow, intRow);
          return false;
        }
      }
      ...
      if (stringCustomName.length() == 0) {
        jtableTable1.setRowSelectionInterval(intRow, intRow);
        message("客戶第:" + (intRow + 1) + "列之[訂戶姓名] 不可空白");
        return false;
      }
      ...
      if (stringPercentage.length() == 0) {
        jtableTable1.setRowSelectionInterval(intRow, intRow);
        message("客戶第 " + (intRow + 1) + "列之[比例%] 不可空白");
        return false;
      }
      if (Float.parseFloat(stringPercentage.trim()) < 1) {
        jtableTable1.setRowSelectionInterval(intRow, intRow);
        message("客戶第 " + (intRow + 1) + "列之[比例%] 不可小於 1");
        return false;
      }
      ...
      if (stringAddress.length() == 0) {
        jtableTable1.setRowSelectionInterval(intRow, intRow);
        message("客戶第 " + (intRow + 1) + "列之[地址] 不可空白");
        return false;
      }
      //
      if (stringTel.length() == 0) {
        jtableTable1.setRowSelectionInterval(intRow, intRow);
        message("客戶第 :" + (intRow + 1) + "-列之[電話] 不可空白");
        return false;
      }
