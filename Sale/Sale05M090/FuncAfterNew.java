...
public class FuncAfterNew extends bNotify {
  public void actionPerformed(String value) throws Throwable {
    ...
    stringSQL = "SELECT ProjectID1,   OrderDate,   SaleID1,        SaleName1,   SaleID2, " + " SaleName2,  SaleID3,        SaleName3,  SaleID4,         SaleName4, "
        + " SaleID5,        SaleName5,  MediaID,       MediaName,  ZoneID, " + "  ZoneName,  MajorID,       MajorName,  UseType,       Remark, "
        + "  SaleID6,        SaleName6, SaleID7,        SaleName7,    SaleID8,  " + "  SaleName8,  SaleID9,       SaleName9, SaleID10,         SaleName10, " + // 增加售出人 20090525
        " SSMediaID,    SSMediaID1 " + // 增加 [媒體細項代碼] 楊信義 2015/05/25
        " FROM Sale05M090 " + " WHERE OrderNo = '" + getValue("OrderNo").trim() + "'";
    ...
    //
    stringSQL = "SELECT CustomName " + " FROM Sale05M091 " + " WHERE OrderNo = '" + getValue("OrderNo").trim() + "'" + " AND ISNULL(StatusCd,'')<>'C' " + " ORDER BY RecordNo ";
    ...
    for (int intSale05M091 = 0; intSale05M091 < retSale05M091.length; intSale05M091++) {
      if (intSale05M091 == 0) stringCustom = retSale05M091[intSale05M091][0];
        ...
        stringCustom = "-" + stringCustom + retSale05M091[intSale05M091][0];
    }
    ...
    stringSQL = "SELECT HouseCar, " + " Position, " + " DealMoney, " + " TrxDate, " + " StatusCd, " + " GiftMoney, " + " CommMoney, " + " ViMoney, " + " CommMoney1 " + // 增加 [中原佣金]
                                                                                                                                                                        // 楊信義
                                                                                                                                                                        // 2015/10/13
        " FROM Sale05M092 " + " WHERE OrderNo = '" + getValue("OrderNo").trim() + "'" + " AND ISNULL(StatusCd,'')<>'D' " + " ORDER BY HouseCar DESC,RecordNo";
    ...
    for (int intSale05M092 = 0; intSale05M092 < retSale05M092.length; intSale05M092++) {
      ...
      String stringCommMoney1 = retSale05M092[intSale05M092][8].trim(); // 增加 [中原佣金] 楊信義 2015/10/13
      //
      String stringA_Sale = " A_Sale";
      String stringASaleMan = "A_Sale_SaleID"; // 2015-10/15 B3018
      stringSQL = "SELECT ID1 " + " FROM " + stringA_Sale + " WHERE ProjectID1 = '" + stringProjectID1 + "'";
      ...
      // 價目表
      stringSQL = "SELECT H_Com, " + " L_Com, " + " PingSu, " + " ListPrice, " + " H_ListPrice, " + " L_ListPrice, " + " FloorPrice, " + " H_FloorPrice, " + " L_FloorPrice "
          + " FROM Sale05M040 " + " WHERE ProjectID1 = '" + stringProjectID1 + "'" + " AND HouseCar = '" + stringHouseCar + "'" + " AND Position = '" + stringPosition + "'";
      ...
      stringSQL = "UPDATE " + stringA_Sale + " SET YearMM = '" + stringOrderDate + "'," + " Com = '" + stringH_Com + "'," + " ProjectID = '" + stringProjectID + "',"
          + " ProjectID0 = '" + stringProjectID1 + "'," + " ProjectID1 = '" + stringProjectID1 + "'," + " H_Com = '" + stringH_Com + "'," + " H_LandOwner = 'N',"
          + " H_LandShare = 'N'," + " L_Com = '" + stringL_Com + "'," + " L_LandOwner = 'N'," + " L_LandShare = 'N'," + " SaleID1  =  '" + retSale05M090[0][2].trim() + "', "
          + " SaleName1  =  '" + retSale05M090[0][3].trim() + "', " + " SaleID2  =  '" + retSale05M090[0][4].trim() + "', " + " SaleName2  =  '" + retSale05M090[0][5].trim()
          + "', " + " SaleID3  =  '" + retSale05M090[0][6].trim() + "', " + " SaleName3  =  '" + retSale05M090[0][7].trim() + "', " + " SaleID4  =  '" + retSale05M090[0][8].trim()
          + "', " + " SaleName4  =  '" + retSale05M090[0][9].trim() + "', " + " SaleID5  =  '" + retSale05M090[0][10].trim() + "', " + " SaleName5  =  '"
          + retSale05M090[0][11].trim() + "', " + " SaleID6  =  '" + retSale05M090[0][20].trim() + "', " + // 增加售出人 20090414
          " SaleName6  =  '" + retSale05M090[0][21].trim() + "', " + // 增加售出人 20090414
          " MediaID  =  '" + retSale05M090[0][12].trim() + "', " + " MediaName  =  '" + retSale05M090[0][13].trim() + "', " + " ZoneID  =  '" + retSale05M090[0][14].trim() + "', "
          + " ZoneName  =  '" + retSale05M090[0][15].trim() + "', " + " MajorID  =  '" + retSale05M090[0][16].trim() + "', " + " MajorName  =  '" + retSale05M090[0][17].trim()
          + "', " + " UseType  =  '" + retSale05M090[0][18].trim() + "', " + " Remark  =  '" + retSale05M090[0][19].trim() + "', ";
      if (stringHouseCar.equals("House")) stringSQL = stringSQL + " Position = '" + stringPosition + "',";
      else {
        stringSQL = stringSQL + " Car = '" + stringPosition + "'," + " Position1 = '" + stringPosition1 + "'," + " Custom1 = '" + stringCustom + "',";
      }
      ...
      if ("".equals("stringCommMoney1")) stringCommMoney1 = "0	"; // 2015-10-13 中原佣金
      if ("".equals("stringViMoney")) stringViMoney = "0	";
      String stringPureMoney = ""
          + (Double.parseDouble(stringDealMoney) - Double.parseDouble(stringGiftMoney) - Double.parseDouble(stringCommMoney) - Double.parseDouble(stringCommMoney1) - // 2015-10-22
                                                                                                                                                                      // B3018
              Double.parseDouble(stringViMoney));
      stringSQL = stringSQL + " Custom = '" + stringCustom + "'," + " OrderDate = '" + stringOrderDate + "'," + " PingSu = " + stringPingSu + "," + " PreMoney = " + stringListPrice
          + "," + " H_PreMoney = " + convert.FourToFive(stringH_ListPrice, 4) + "," + " L_PreMoney = " + convert.FourToFive(stringL_ListPrice, 4) + "," + " DealMoney = "
          + convert.FourToFive(stringDealMoney, 4) + "," + " H_DealMoney = " + convert.FourToFive("" + Double.parseDouble(stringDealMoney) * doubleHouseRate, 4) + ","
          + " L_DealMoney = " + convert.FourToFive("" + Double.parseDouble(stringDealMoney) * doubleLandRate, 4) + "," + " GiftMoney = " + convert.FourToFive(stringGiftMoney, 4)
          + "," + " H_GiftMoney = " + convert.FourToFive("" + Double.parseDouble(stringGiftMoney) * doubleHouseRate, 4) + "," + " L_GiftMoney = "
          + convert.FourToFive("" + Double.parseDouble(stringGiftMoney) * doubleLandRate, 4) + "," + " CommMoney = " + convert.FourToFive(stringCommMoney, 4) + ","
          + " H_CommMoney = " + convert.FourToFive("" + Double.parseDouble(stringCommMoney) * doubleHouseRate, 4) + "," + " L_CommMoney = "
          + convert.FourToFive("" + Double.parseDouble(stringCommMoney) * doubleLandRate, 4) + "," + " CommMoney1 = " + convert.FourToFive(stringCommMoney1, 4) + "," + // 2015-10-13
                                                                                                                                                                        // B3018中原佣金
          " H_CommMoney1 = " + convert.FourToFive("" + Double.parseDouble(stringCommMoney1) * doubleHouseRate, 4) + "," + // 2015-10-13 B3018中原佣金
          " L_CommMoney1 = " + convert.FourToFive("" + Double.parseDouble(stringCommMoney1) * doubleLandRate, 4) + "," + // 2015-10-13 B3018中原佣金
          " ViMoney = " + convert.FourToFive(stringViMoney, 4) + "," + " H_ViMoney = " + convert.FourToFive("" + Double.parseDouble(stringViMoney) * doubleHouseRate, 4) + ","
          + " L_ViMoney = " + convert.FourToFive("" + Double.parseDouble(stringViMoney) * doubleLandRate, 4) + "," + " PureMoney = " + stringPureMoney + "," + " H_PureMoney = "
          + convert.FourToFive("" + Double.parseDouble(stringPureMoney) * doubleHouseRate, 4) + "," + " L_PureMoney = "
          + convert.FourToFive("" + Double.parseDouble(stringPureMoney) * doubleLandRate, 4) + "," + " LastMoney = " + stringFloorPrice + "," + " H_LastMoney = "
          + stringH_FloorPrice + "," + " L_LastMoney = " + stringL_FloorPrice + "," + " BalaMoney = "
          + convert.FourToFive("" + (Double.parseDouble(stringPureMoney) - Double.parseDouble(stringFloorPrice)), 4) + "," + " H_BalaMoney = "
          + convert.FourToFive("" + (Double.parseDouble(stringPureMoney) * doubleHouseRate - Double.parseDouble(stringH_FloorPrice)), 4) + "," + " L_BalaMoney = "
          + convert.FourToFive("" + (Double.parseDouble(stringPureMoney) * doubleLandRate - Double.parseDouble(stringL_FloorPrice)), 4) + "," + " OrderNo = '"
          + getValue("OrderNo").trim() + "', " + " SSMediaID  =  '" + retSale05M090[0][30].trim() + "', " + // 增加[媒體代碼 2010/04/21
          " SSMediaID1  =  '" + retSale05M090[0][31].trim() + "', " + // 增加[媒體細項代碼 2015/05/25
      ...
      //
      stringSQL = exeUtil.doDeleteDB(stringASaleMan, new Hashtable(), " AND  ID1  =  " + stringID1 + " ", false, dbSale);
      vectorSql.add(stringSQL);
      for (int intNo = 0; intNo < vectorSale05M421.size(); intNo++) {
        hashtableDB = (Hashtable) vectorSale05M421.get(intNo);
        if (hashtableDB == null) continue;
        //
