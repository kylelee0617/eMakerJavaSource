...
public class Data220624 extends bproc {
  String 史大Date;
  String 安得Date;
  KSqlUtils ksUtil;
  KUtils kUtil;
  ...
  private void 執行() throws Throwable {
    ...
    String[] title = { "訂單編號", "案別", "棟樓別" , "車位別" ,"客戶姓名" ,"客戶ID" ,"訂單日期" ,"簽約日期" ,"交屋日期" ,"退戶日" ,"退戶原因" //0-10
        ...
        ,"新客戶背景" ,"新往來通路" ,"新相關地域" ,"新往來產品" ,"新風險計算" ,"新風險等級"};                                                  //17-22
    ...
    for (int i = 0; i < ret.length; i++) {
      ...
      //沒有行業別的人們
      String sql2 = "select CMTIDF, CMNAME ,CVOCAT ,CVOCAT ,CMLUPY ,CMLUPM ,CMLUPD "
                  + "from PLSPFLIB.CMSCLNTM "
                  + "where CMTIDF = '" + customNo + "' "
                  + "order by CMLUPY desc , CMLUPM desc , CMLUPD desc "
                  + "FETCH FIRST 1 ROWS ONLY ";
      String[][] ret2 = dbAS400.queryFromPool(sql2);
      if (ret2.length == 0 || StringUtils.isBlank(ret2[0][2].trim())) {
        //抓舊的風險分數
        String sql3 = "select RO0201, RO0205, RO0207, RO0203, RO0209, RO0210, RI02 " 
                    ...
                    + "and RIFILE = '"+orderNo+"' "
                    + "and RI04 = 'RY' "
                    + "and RI02 > 1081204 and RI02 < 1101002 " 
