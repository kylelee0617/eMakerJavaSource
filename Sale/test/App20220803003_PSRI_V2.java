 ...
 *
 */
public class App20220803003_PSRI_V2 extends bproc {
  String 史大Date;
  String 安得Date;
  KSqlUtils ksUtil;
  KUtils kUtil;
  ...
  private void 執行() throws Throwable {
    ...
    try {
      ...
      while(true) {
        ...
        String yyyy = StringUtils.isNumeric(orderDateArr[0])? ""+(Integer.parseInt(orderDateArr[0])-1911):"";
        String MM = orderDateArr[1].trim().length()>1? orderDateArr[1].trim():"0"+orderDateArr[1].trim();
        String dd = orderDateArr[2].trim().length()>1? orderDateArr[2].trim():"0"+orderDateArr[2].trim();
        ...
        String sql4 = "INSERT INTO PSRI02PF "
                    ...
                    + "VALUES "
                    + "('"+ custId +"', "+ yyyy+MM+dd +", 150410, 'RY', 'QUSER', '', '"+ orderNo +"', '15', 'RYB', '1', '"+ nationCode +"', '', '', "
                    + "15, '中風險', 30, '高風險', 10, '低風險', "+ nationRiskVal +", '"+ nationRiskName +"', 16, '"+ targetRiskName +"', "
                    + "'"+ custId +"', '', '', '') ";
