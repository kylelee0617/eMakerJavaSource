...
public class Sale02R030 extends bTransaction {
  ...
  public boolean doSaleOutDetailDelete(String stringExcelName, String stringTableSelect) throws Throwable {
    ...
    for (int intNo = 0; intNo < retAProjectLocal.length; intNo++) {
      ...
      if ("".equals(stringHouseCar)) {
        ...
        if (!"".equals(stringDistributeNo)) {
          if (stringHComLocal.equals(stringDistributeNo)) booleanHouse = true; // 房子累加
          if (stringLComLocal.equals(stringDistributeNo)) booleanLand = true; // 土地累加
        } else {
        ...
        // 房
        if ("H_".equals(stringHouseCar)) if ("".equals(stringDistributeNo) || stringHComLocal.equals(stringDistributeNo)) booleanHouse = true; // 房子累加
        // 土
        if ("L_".equals(stringHouseCar)) if ("".equals(stringDistributeNo) || stringLComLocal.equals(stringDistributeNo)) booleanLand = true; // 土地累加
      }
      //
      if ("房子".equals(stringThisType)) {
        if (booleanHouse) double可售金額 += doParseDouble(retAProjectLocal[0][4].trim(), "1-1");// PositionMoneyS2
        if (booleanLand) double可售金額 += doParseDouble(retAProjectLocal[0][5].trim(), "1-2");// PositionMoneyS3
      } else if ("車位".equals(stringThisType)) {
        if (booleanHouse) double可售金額 += doParseDouble(retAProjectLocal[0][7].trim(), "1-4");// CarMoneyS2
        if (booleanLand) double可售金額 += doParseDouble(retAProjectLocal[0][8].trim(), "1-5");// CarMoneyS3
      }
    }
    ...
    // 退戶率
    double double退戶率 = 0;
    if (double可售金額 != 0) double退戶率 = double售價累計 / double可售金額;
...
//    System.out.println("stringThisType>>>" + stringThisType);
    ...
    for (int intNo = 0; intNo < retAProject.length; intNo++) {
      ...
      if ("房子".equals(stringThisType)) {
        if (booleanHouse) double可售金額3 += doParseDouble(retAProject[intNo][3].trim(), "2"); // PositionMoneyS2
        if (booleanLand) double可售金額3 += doParseDouble(retAProject[intNo][4].trim(), "3"); // PositionMoneyS3
      } else if ("車位".equals(stringThisType)) {
        if (booleanHouse) double可售金額3 += doParseDouble(retAProject[intNo][6].trim(), "5"); // CarMoneyS2
        if (booleanLand) double可售金額3 += doParseDouble(retAProject[intNo][7].trim(), "6"); // CarMoneyS3
      }
    }
    ...
    for (int intNo = 0; intNo < retSumDealMoneyForASale.length; intNo++) {
      ...
      if (booleanHouse) double累計成交單價 += doParseDouble(retSumDealMoneyForASale[intNo][3], "43"); // SumDealMoney2
      if (booleanLand) double累計成交單價 += doParseDouble(retSumDealMoneyForASale[intNo][4], "44"); // SumDealMoney3
    }
    if (double坪數 == 0) {
