...
public class KSqlUtils extends bproc {
   ...
   * @throws Throwable
   */
  public String[] getEMAIL收件者名單(String eMakerUserNo, boolean isTest) throws Throwable{
...
//        DPeMail2 = reteMail[0][1];
  ...
  public boolean insSaleLog(SaleLogBean bean) throws Throwable {
    boolean rs = false;
    String sql = "INSERT INTO SaleLog (FuncName, RecordType, EmpNo, FgEmpNo, LogDesc, UpdateDate, Type1, Type2, Type3) "
               + "VALUES "
               + "('"+bean.getFuncName()+"', '"+bean.getRecordType()+"', '"+bean.getEmpNo()+"', '"+bean.getFgEmpNo()+"', "
                   + "N'"+bean.getLogDesc()+"', '"+bean.getUpdateDate()+"', '"+bean.getType1()+"', '"+bean.getType2()+"', '"+bean.getType3()+"') ";
    try {
  ...
  */
  ...
  public QueryLogBean getQueryLogLike3(String projectId, String custNo) throws Throwable {
    ...
    String sql = "select top 1 "
        + "QID, REASON, PROJECT_ID, NATIONAL_ID, QUERY_TYPE, NTCODE, NAME, QUERY_ID, BIRTHDAY, SEX, JOB_TYPE, CITY, "
        ...
        + "NTCODE2, ENG_NAME, ENG_NO, QUERY_ID3 "
        + "from query_log a where a.PROJECT_ID = '" + projectId + "' and a.QUERY_ID3 like '%" + custNo.toUpperCase() + "%' order by CREATE_DATE desc , CREATE_TIME desc ";
