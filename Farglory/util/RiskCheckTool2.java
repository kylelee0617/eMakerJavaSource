...
public class RiskCheckTool2 extends bvalidate {
  ...
  public RiskCheckTool2(RiskCheckBean bean) throws Throwable {
    
    System.out.println(">>>RiskCheckTool2 init");
  ...
  public Result processRisk() throws Throwable {
    ...
    try {
      ...
      for (int i = 0; i < retCustom.length; i++) { // start 客戶for
        ...
        System.out.println("RTCODEA:" + ra.getResult()[22]);
        
        boolean b = rb.invoke(as400init, blpc00a, mapb);
        System.out.println("RTCODEB:" + rb.getResult()[6]);
      ...
      rcRS.setRsMsg("錯誤訊息:" + e.toString());
      rs.setData(rcRS);
      
      rs.setRsStatus(rsStatus.ERROR);
      rs.setReturnMsg(e.toString());
    }
