...
//    System.out.println(">>cms00c: " + cms00c);
    ...
    try {
      ...
      for(int i=0 ; i<cBeans.length ; i++) {
        ...
        //行業別
        String indCode = "";
        if(StringUtils.isNotBlank(cBean.getIndustryCode())) {
          indCode = cBean.getIndustryCode();
        }else if(StringUtils.isNotBlank(cBean.getMajorName())) {
          indCode = kSqlUtil.getIndustryCodeByMajorName(cBean.getMajorName());
        }
        System.out.println("行業別:" + indCode + " / " + kSqlUtil.getNameByIndCode(indCode));
