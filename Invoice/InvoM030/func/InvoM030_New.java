 ...
 * PT1527822019699
 *
 * Tip: 這邊開的是行銷的發票( 哈扣ProcessInvoiceNo=1 )
 * 
 * @author B04391
 *
 */
...
public class InvoM030_New extends bTransaction {
  public boolean action(String value) throws Throwable {
    ...
    stringSQL = "SELECT TOP 1 InvoiceYYYYMM, FSChar, StartNo, InvoiceBook, InvoiceStartNo, InvoiceEndNo, MaxInvoiceNo, "
        ...
        + "' AND (MaxInvoiceDate <= '" + getValue("InvoiceDate").trim() + "' OR MaxInvoiceDate IS NULL OR LEN(MaxInvoiceDate) = 0) AND ENDYES = 'N' "
        + " AND CloseYes = 'N'  AND ProcessInvoiceNo = '" + processNo + "'";
    ...
    for (int i = 0; i < retInvoM022.length; i++) { // 就top 1 了你跑毛迴圈阿，還是你覺得top 1 不會只有一筆? (by Kyle to 不知名神人)
    ...
    try {
      ...
      sbSQL.append("and ENDYES='").append(stringEndYes).append("' ");
