...
public class Doc2M014 extends bTransaction {
  ...
  public boolean action(String value) throws Throwable {
    ...
    if (!isBatchCheckOK(booleanSource272, retDoc5M0224, retDoc2M010, booleanSource, booleanComNoType, exeFun, exeUtil, hashtableAsset, vectorRentSql)) {
    ...
    if ("修改".equals(value.trim())) {
      ...
      if (",B,".indexOf(stringVoucherType) != -1) {
        String[][] retDoc2M0143 = exeFun
            .getTableDataDoc("SELECT  COMPANY_CD,  VOUCHER_YMD,  VOUCHER_FLOW_NO  FROM  Doc2M0143  WHERE  BarCode  =  '" + stringBarCode + "'  AND  STATUS_CD = 'Z'");
        String stringTemp = "";
        stringTemp += retDoc2M0143[0][1].trim().replaceAll("/", "") + "-" + convert.add0(retDoc2M0143[0][2].trim(), "5");
  ...
  public void doUpdateDoc1M030(Doc.Doc2M010 exeFun, FargloryUtil exeUtil) throws Throwable {
    ...
    // exeUtil.doEMail(stringSubject, "原本公文內容"+stringDescriptB+"<br>"+stringSql,
    // stringSend, arrayUser) ;
  }

  public void doInsertDB(boolean booleanSource, boolean booleanComNoType, String[][] retDoc2M014, String[][] retDoc5M0224, String[][] retDoc2M010, Hashtable hashtableAsset,
      Doc.Doc2M010 exeFun, FargloryUtil exeUtil, Vector vectorRentSql) throws Throwable {
    ...
    try {
      ...
      for (int intNo = 0; intNo < retDoc2M014.length; intNo++) {
        ...
        if (!booleanCostID0010 && retDoc5M0224.length > 0 && "N".equals(stringRowType)) {
          ...
          if (",J35648,".indexOf(stringBarCode) == -1) {
            // 新增 FED1014
            stringSql = exeFun.getInsertFED1014Sql(stringVoucherYMDL, stringVoucherFlowNoL, stringVoucherSeqNoL, stringCompanyCd, stringKind, stringVoucherYMDRoc,
                stringVoucherFlowNo, "" + intVoucherSeqNo, stringAmtL, " ", stringUserN);
            vectorSql.add(stringSql);
            System.out.println(intNo + "立沖  (FED1014)---------" + stringSql);
            // 更新 FED1013
            stringSql = exeFun.getUpdateFED1013Sql(stringAmtL, stringVoucherYMDL, stringVoucherFlowNoL, stringVoucherSeqNoL, stringCompanyCd, stringKind);
        ...
        if ("Y".equals(retFED1003[0][2].trim())) {
          if (stringDBCrCd.equals(stringDBCrCd1003)) {
            if (",J35648,".indexOf(stringBarCode) == -1) {
              // FED1013 處理
              stringSql = exeFun.getInsertFED1013Sql(stringVoucherYMDRoc, stringVoucherFlowNo, "" + intVoucherSeqNo, stringCompanyCd, stringKind, stringAccountNo, stringObjectCd,
                  " ", "0", "0", "0", "0");
              vectorSql.add(stringSql);
              ...
              if (retDoc5M02201.length == 0) {
                if (booleanCostID0010) {
                  ...
                  if (retDoc2M015.length > 0) {
                    retDoc2M012 = exeFun.getDoc2M012Union(booleanSource ? "Doc2M012" : "Doc5M022", stringBarCode, retDoc2M015[0][0].trim(), retDoc2M015[0][1].trim(),
                        retDoc2M015[0][3].trim(), retDoc2M015[0][4].trim(), retDoc2M015[0][5].trim());
                    stringSql = exeFun.getUpdateFED1013Sql("" + doubleAMT, stringVoucherYMDRoc, stringVoucherFlowNo, retDoc2M012[0][8].trim(), stringCompanyCd, stringKind);
                    /*
                     * stringSql = exeFun.getInsertFED1013Sql(stringVoucherYMDRoc,
                     * stringVoucherFlowNo, "1", stringCompanyCd, stringKind, stringAccountNo,
                     * stringObjectCd, " ", "0", ""+doubleAMT, "0", "0") ;
                     */
                    vectorSql.add(stringSql);
                    System.out.println(intNo + "(FED1013)---------" + stringSql);
                    // 新增 FED1014
                    stringSql = exeFun.getInsertFED1014Sql(stringVoucherYMDRoc, stringVoucherFlowNo, retDoc2M012[0][8].trim(), stringCompanyCd, stringKind, stringVoucherYMDRoc,
                        stringVoucherFlowNo, "" + intVoucherSeqNo, "" + doubleAMT, " ", stringUserN);
                    vectorSql.add(stringSql);
                ...
                if (!booleanSource) {
                  ...
                  if (intNo < retDoc5M02201.length) {
                    ...
                    if (",J35648,".indexOf(stringBarCode) == -1) {
                      // 新增 FED1014
                      stringSql = exeFun.getInsertFED1014Sql(stringVoucherYMDL, stringVoucherFlowNoL, stringVoucherSeqNoL, stringCompanyCdL, stringKindL, stringVoucherYMDRoc,
                          stringVoucherFlowNo, "" + intVoucherSeqNo, stringBackRetainMoneyL, " ", stringUserN);
                      vectorSql.add(stringSql);
                      System.out.println(intNo + "(FED1014)---------" + stringSql);
                      // 更新 FED1013
                      stringSql = exeFun.getUpdateFED1013Sql(stringBackRetainMoneyL, stringVoucherYMDL, stringVoucherFlowNoL, stringVoucherSeqNoL, stringCompanyCdL, stringKindL);
                      vectorSql.add(stringSql);
                      System.out.println(intNo + "(FED1013)---------" + stringSql);
                    }
                  }
                  ...
                  if (retDoc2M014L.length == 0) {
                    // E-mail 通知
                    stringMessage = "此次轉傳票條碼編號：" + stringBarCode + "，退保留款條碼編號：" + stringRetainBarCode + " <br> 無法沖帳<br>查無對應之原保留款傳票資訊(Doc2M014)。";
                    exeUtil.doEMail(stringSubject, stringMessage, stringSend, arrayUser);
                  }
                  ...
                  if (retFED1012L.length == 0 || !stringAcctNoL.equals(retFED1012L[0][1].trim())) {
                    ...
                    stringMessage = "此次轉傳票條碼編號：" + stringBarCode + "，退保留款條碼編號：" + stringRetainBarCode + " <br> 無法沖帳<br>查無對應之原保留款傳票資訊(FED1012)。";
                    exeUtil.doEMail(stringSubject, stringMessage, stringSend, arrayUser);
                  }
                  // 新增 FED1014
                  stringSql = exeFun.getInsertFED1014Sql(stringVoucherYMDL, stringVoucherFlowNoL, stringVoucherSeqNoL, stringCompanyCdL, stringKindL, stringVoucherYMDRoc,
                  ...
                  System.out.println(intNo + "(FED1014)---------" + stringSql);
                  // 更新 FED1013
                  stringSql = exeFun.getUpdateFED1013Sql("" + doubleAMT, stringVoucherYMDL, stringVoucherFlowNoL, stringVoucherSeqNoL, stringCompanyCdL, stringKindL);
                  vectorSql.add(stringSql);
                  System.out.println(intNo + "(FED1013)---------" + stringSql);
                  // 更新狀態
                  ...
                  stringSql = " UPDATE  Doc2M010  SET  WriteRetainMoney  =  " + stringWriteRetainMoney + " " + " WHERE BarCode  =  '" + stringRetainBarCode + "' ";
        ...
        // FED1012 處理
        stringSql = exeFun.getInsertFED1012Sql(stringVoucherYMDRoc, stringVoucherFlowNo, "" + intVoucherSeqNo, retDoc2M014[intNo][7].trim(), stringKind,
            retDoc2M014[intNo][9].trim(), retDoc2M014[intNo][10].trim(), stringDeptCd, retDoc2M014[intNo][12].trim(), retDoc2M014[intNo][13].trim(), "0",
            retDoc2M014[intNo][15].trim(), retDoc2M014[intNo][17].trim(), retDoc2M014[intNo][18].trim(), retDoc2M014[intNo][19].trim(), retDoc2M014[intNo][20].trim(),
            retDoc2M014[intNo][21].trim(), retDoc2M014[intNo][22].trim(), retDoc2M014[intNo][23].trim(), stringUserN);
        ...
        if (!boolean134O1 && "134O1".equals(stringDeptCd)) {
           ...
           * hashtableData.put("MailKEY",
           * "FED1158通知"+stringComNoL+"-"+stringVoucherYMDRoc+"-"+stringVoucherFlowNo) ;
           * hashtableData.put("EmployeeNo", getUser()) ; hashtableData.put("MailTitle",
           * "FED1158通知") ; hashtableData.put("MailContent",
           * "請款<br>"+stringBarCode+"<br>"+stringComNoL+"-"+stringVoucherYMDRoc+"-"+
           * stringVoucherFlowNo) ; hashtableData.put("MailUserID", "B3018") ; stringSql =
           * exeFun.doInsertDBDoc("Doc7M004", hashtableData, false, exeUtil) ;
           * vectorDocSql.add(stringSql) ;
           */
        }
        // 折讓
        ...
        if ("D".equals(stringRowType.substring(0, 1))
            && (intNo + 1 == retDoc2M014.length || (!"H".equals(retDoc2M014[intNo + 1][2].trim()) && !"D".equals(retDoc2M014[intNo + 1][2].trim())))) {
          // 無折讓稅額之折讓單
      ...
      String[][] retData = exeFun.getTableDataDoc(
          "SELECT  ID_Def,  UseMoney " + " FROM  Doc2M0801 " + " WHERE  ID_BarCode  =  " + stringID + " " + " AND  ComNo  =  '" + stringCompanyCd + "' " + " ORDER BY ID_Def  ");
        ...
        if (booleanSource && retData.length > 0) {
          if (!booleanExcept) {
            ...
            for (int intVector = 0; intVector < vectorYOrN.size(); intVector++) {
              ...
              // ("+exeUtil.doParseInteger(""+intVoucherSeqNoEndYear)+")") ;
              ...
              // ;
              // 新增 FED1014
              stringSql = exeFun.getInsertFED1014Sql(stringVoucherDateEndYear, stringVoucherFlowNoEndYear, "" + intVoucherSeqNoEndYear, stringCompanyCd, stringKind,
                  stringVoucherYMDRoc, stringVoucherFlowNo, stringVoucherSeqNo, stringAMTYearEnd, " ", stringUserN);
              vectorSql.add(stringSql);
              System.out.println("新增 FED1014---------------------" + stringSql);
              // 更新 FED1013
              stringSql = exeFun.getUpdateFED1013Sql(stringAMTYearEnd, stringVoucherDateEndYear, stringVoucherFlowNoEndYear, "" + intVoucherSeqNoEndYear, stringCompanyCd,
                  stringKind);
              vectorSql.add(stringSql);
              ...
              if (retFED1012L.length == 0) {
                // stringYearMessage += "此次轉傳票條碼編號(" + stringBarCode + ")，找無欲
                // 年底預估沖銷之傳票資料(FED1012)。\n" ;
              ...
              stringSql = "(I.VOUCHER_YMD  =  " + stringVoucherDateEndYear + "  AND  " + " I.VOUCHER_FLOW_NO  =  " + stringVoucherFlowNoEndYear + "  AND "
              ...
              if (retFED1013.length == 0) {
                ...
                if (exeUtil.doParseDouble(stringCanWriteAmt) < exeUtil.doParseDouble(stringAMTYearEnd)) {
                  stringYearMessage += "此次轉傳票請款條碼編號(" + stringBarCode + ")，欲年底預估沖銷之傳票可沖銷金額(" + exeUtil.getFormatNum2(stringCanWriteAmt) + ")小於本次沖銷金額("
                      + exeUtil.getFormatNum2(stringAMTYearEnd) + ")。\n";
                  booleanYearFlag = true;
                }
              }
          ...
          if (exeFun.getTableDataDoc("SELECT  BarCode  FROM  Doc2M0801  WHERE  BarCode  =  '" + stringBarCode + "' ").length == 0) {
            stringSql = "UPDATE  Doc2M080  SET  Status  =  'Y' " + " WHERE  ID_Def  IN  (SELECT  ID  FROM  " + (booleanSource ? "Doc2M010" : "Doc5M020") + "  WHERE  BarCode  =  '"
                + stringBarCode + "') " + " AND  Remark  LIKE  '%請款%'";
      ...
      if (vectorSql.size() > 0) {
        /*
         * if(booleanComNoType) { talk dbFED1A = getTalk("" +get("put_FED1A")); // 基金會
         * dbFED1.execFromPool((String[]) vectorSql.toArray(new String[0])) ;
         * exeFun.doErrorEmail("基金會----批次轉傳票") ; } else {
         */
  ...
  public void doBackDoc5M0229(String stringCompanyCd, String stringVoucherYMDRoc, String stringVoucherFlowNo, String stringBarCode, FargloryUtil exeUtil, Doc2M010 exeFun)
      throws Throwable {
    ...
    String[][] retProprContractField = { { "company_cd", "voucher_ymd", "voucher_flow_no", "permit" }, { "add1_company_cd", "add1_voucher_ymd", "add1_voucher_flow_no", "permit1" },
        { "add2_company_cd", "add2_voucher_ymd", "add2_voucher_flow_no", "permit2" }, { "add3_company_cd", "add3_voucher_ymd", "add3_voucher_flow_no", "permit3" },
    ...
    for (int intNo = 0; intNo < vectorDoc5M0229.size(); intNo++) {
      hashtableDoc5M0229 = (Hashtable) vectorDoc5M0229.get(intNo);
      if (hashtableDoc5M0229 == null) continue;
  ...
  public Vector getWriteVourcherData(String stringCompanyCd, String stringID, String[][] retDoc2M080, Doc2M010 exeFun, FargloryUtil exeUtil, Vector vectorVoucherData)
      throws Throwable {
  ...
  public Vector getWriteVourcherDataNew(Vector vectorDoc2M0803, Doc2M010 exeFun, FargloryUtil exeUtil, Vector vectorVoucherData) throws Throwable {
    ...
    for (int intNo = vectorDoc2M0803.size() - 1; intNo >= 0; intNo--) {
      ...
      if (hashtableDoc2M0803 == null) continue;
  ...
  public void doSalaryUpdate(String stringCompanyCdO, String stringVoucherYMDRoc, String stringVoucherFlowNo, String stringUserN, Doc2M010 exeFun, FargloryUtil exeUtil,
      Vector vectorSql, Vector vectorDocSql) throws Throwable {
    String stringVoucherType = getValue("VoucherType").trim();
    if (",B,C,".indexOf(stringVoucherType) == -1) return;
  ...
  public boolean doCostID0010TypeD(String[] arrayVouceherData, Hashtable hashtableVoucherAmt, Doc2M010 exeFun, FargloryUtil exeUtil, Vector vectorSql) throws Throwable {
    ...
    // 依 公司+會計科目+部門+廠商 沖銷
    String stringSqlAnd = " AND  J.ACCT_NO  =  '" + stringAcctNo + "' " + " AND  J.DEPT_CD  =  '" + stringDeptCd + "' " + " AND  J.OBJECT_CD  =  '" + stringObjectCd + "' "
    ...
    for (int intNo = 0; intNo < retFED1013.length; intNo++) {
      ...
      // 新增 FED1014
      stringSql = exeFun.getInsertFED1014Sql(stringVoucherYMDL, stringVoucherFlowNoL, stringVoucherSeqNoL, stringComNo, stringKind, stringVoucherYMDRoc, stringVoucherFlowNo,
          stringVoucherSeqNo, stringAmtL, " ", stringUserN);
      vectorSql.add(stringSql);
      System.out.println(intRowNo + "立沖  (getInsertDoc2M014Sql)(" + stringAmtL + ")---------" + stringSql);
      // 更新 FED1013
      stringSql = exeFun.getUpdateFED1013Sql(stringAmtL, stringVoucherYMDL, stringVoucherFlowNoL, stringVoucherSeqNoL, stringComNo, stringKind);
      System.out.println(intRowNo + "立沖  (getInsertDoc2M014Sql)(" + stringAmtL + ")---------" + stringSql);
      vectorSql.add(stringSql);
      //
  ...
  public boolean doCostID0010TypeC(String[] arrayVouceherData, String[][] retDoc5M0224, Hashtable hashtableCanWriteAmt, Doc2M010 exeFun, FargloryUtil exeUtil, Vector vectorSql)
      throws Throwable {
    int intRowNo = exeUtil.doParseInteger(arrayVouceherData[0].trim());
    int intDBCrCdC = exeUtil.doParseInteger(arrayVouceherData[1].trim());
    String stringComNo = arrayVouceherData[2].trim();
  ...
  // 檢核
  // 前端資料檢核，正確回傳 True
  public boolean isBatchCheckOK(boolean booleanSource272, String[][] retDoc5M0224, String[][] retDoc2M010, boolean booleanSource, boolean booleanComNoType, Doc.Doc2M010 exeFun,
      FargloryUtil exeUtil, Hashtable hashtableAsset, Vector vectorRentSql) throws Throwable {
    //
    int intPositionC = 0;
    String stringAcctNo = "";
    ...
    if (!booleanSource) {
      ...
      for (int intNo = 0; intNo < retDoc2M012L.length; intNo++) {
        ...
        if ("A110202,A110302,A110303,A110401,A110901,A110902,A110903,A110911,A110940,A110941,A110942,".indexOf(stringCostIDL) != -1) {
          booleanA110202 = true;
        }
        if ("A110940,A110941,A110942,A140802,A140803,A140804".indexOf(stringCostIDL) != -1) {
          booleanTable14 = true;
        }
        if (vectorCostIDExcept.indexOf(stringCostIDL) != -1) {
          booleanA110201 = true;
        }
        if ("F297001,".indexOf(stringCostIDL + ",") != -1) {
          booleanF297001 = true;
        }
    ...
    if (booleanRowTypeR) {
      // 薪資轉傳票
      if (vectorDoc2M0143.size() != 2) {
        messagebox("資料錯誤，請洽資訊室。1");
        return false;
      }
      // 一般 A
      if ("A".equals(stringVoucherType)) {
        messagebox("狀態錯誤，請洽資訊室。");
        return false;
      }
      //
      ...
      for (int intNo = 0; intNo < vectorDoc2M0143.size(); intNo++) {
        ...
        if (hashtableTemp == null) continue;
    ...
    for (int intRowNo = 0; intRowNo < jtable1.getRowCount(); intRowNo++) {
      ...
      if (!booleanCostID0010 && "8111,1282,".indexOf(stringAcctNo) == -1 && retDoc5M0224.length > 0 && "D".equals(stringDbCrCd)) {
        ...
        if (",J35648,".indexOf("," + stringBarCode + ",") == -1) {
          if (exeUtil.doParseDouble(stringAmt) != exeUtil.doParseDouble(retDoc5M0224[intRecordNo - 1][5].trim())) {
            message("第 " + (intRowNo + 1) + " 列之[沖銷金額](" + exeUtil.getFormatNum2(stringAmt) + ")  與 [請款金額](" + exeUtil.getFormatNum2(retDoc5M0224[intRecordNo - 1][5].trim())
                + ")  不同。");
            jtable1.setRowSelectionInterval(intRowNo, intRowNo);
            return false;
          }
      ...
      /* 年底預估不做處理 */
      if ("W".equals(stringType)) {
        booleanRowTypeW = true;
        continue;
      }
      //
      ...
      if ("A".equals(stringType)) {
        ...
        // doubleInvoiceTotalMoney += exeFun.doParseDouble( ("" + getValueAt("Table1",
        // intRowNo, "DESCRIPTION_4")).trim( )) ;
      }
      // 折讓(減去特別折讓後須大於零)
      ...
      for (int intNo = 0; intNo < retFED1004.length; intNo++) {
        ...
        // 廣告企劃費代碼
        booleanCostID = !"".equals(stringCostIDPosition) && !"null".equals(stringCostIDPosition) && check.isNum(stringCostIDPosition)
        ...
        if (retFED1002.length != 0) {
          ...
          if ("1264".equals(stringAcctNo) && "B09".equals(stringItemCd) && "A".equals(stringType)) {
            ...
            // if(exeFun.getInvoiceNoDoc2M011(stringBarCode).indexOf(stringUseCd) == -1 &&
            // !exeFun.isExistInvoiceNoCheck(stringUseCd)) {
            String stringFactoryNameL = exeFun.getFactoryName(stringObjectCd, booleanComNoType);
            if ("".equals(stringFactoryNameL)) stringFactoryNameL = "11111";
            if (stringInvoiceKind.indexOf("收據") != -1 || stringInvoiceKind.indexOf("載具") != -1) {
                ...
                || stringFactoryNameL.indexOf("中華電信") != -1 || stringFactoryNameL.indexOf("亞太電信") != -1 || stringFactoryNameL.indexOf("台灣電信") != -1)) {
            } else if (booleanTable14) {
              // 代收代付
              Vector vectorComNo = new Vector();
              vectorComNo.add(stringCompanyCD);
          ...
          if ("1264".equals(stringAcctNo) && "B11".equals(stringItemCd)) {
            ...
            // setValueAt("Table1", convert.FourToFive("" + (doubleInvoiceTax +
            // doubleInvoiceMoney), 0), intRowNo, "DESCRIPTION_"+(intNo+1)) ;
          }
          // 所得給付票據到期日 G08
          ...
          // 所得總額 G20
          if ("A".equals(stringVoucherType) && !"Y".equals(getValue("CountSame")) && "G20".equals(stringItemCd) && "C".equals(stringDbCrCd)
              && "228231,".indexOf(stringAcctNo) == -1) {
            double doubleReceiptTaxRate = exeUtil.doParseDouble(getValue("ReceiptRate").trim()) / 100;
            String[][] retDoc2M013 = null;
            if (booleanSource) {
              // 行銷
              retDoc2M013 = exeFun.getTableDataDoc(
                  " SELECT  ReceiptTaxType " + " FROM  Doc2M013 " + " WHERE  BarCode  = '" + stringBarCode + "' " + " AND  ReceiptTax > 0  " + " ORDER BY  RecordNo");
            } else {
              // 管理
              retDoc2M013 = exeFun.getTableDataDoc(
                  " SELECT  ReceiptTaxType " + " FROM  Doc5M023 " + " WHERE  BarCode  = '" + stringBarCode + "' " + " AND  ReceiptTax > 0  " + " ORDER BY  RecordNo");
            }
            ...
            if (",Z0001,Z8000,".indexOf("," + stringObjectCd + ",") == -1) {
              ...
              if ((doubleReceiptMoneyTax - 3 > doubleReceiptTax) || (doubleReceiptMoneyTax + 3 < doubleReceiptTax)) {
                message("第 " + (intRowNo + 1) + " 列之[扣繳稅額] (" + exeUtil.getFormatNum2("" + doubleReceiptTax) + ") 須為 [所得淨額] 乘以 [稅率] 正負 3 的範圍內。("
                    + exeUtil.getFormatNum2("" + doubleReceiptMoneyTax) + ")");
                jtable1.setRowSelectionInterval(intRowNo, intRowNo);
                if (!booleanRowTypeR) return false;
              }
            }
            // 所得淨額 + 扣繳金額
            // setValueAt("Table1",
            // convert.FourToFive(""+(doubleReceiptTax+doubleReceiptMoney), 0), intRowNo,
            // "DESCRIPTION_"+(intNo+1)) ;
          }
        }
      }
    }
    ...
    String[][] retDoc2M012 = booleanSource ? exeFun.getDoc2M012(stringBarCode) : new String[0][0];
    ...
    if (",B,C,".indexOf("," + stringDocNoType + ",") == -1 && !booleanRowTypeR && !booleanCostIDExcept && !booleanA110201) {
      ...
      if (!booleanSource) {
        doubleInvoiceTotalMoney += exeFun.getInvoiceMoneySum("Doc5M0211", stringBarCode) + exeFun.getInvoiceTaxSum("Doc5M0211", stringBarCode);
      }
      if (!"OK".equals(stringAssetStatus) && doubleAmtC != doubleInvoiceTotalMoney && !booleanCostIDExcept && !booleanPocketMoney && !booleanRowTypeW) {
         ...
         * "借方總金額("+doubleAmtC+") 不等於 請款申請書之發票總金額或個人收據總金額合計("+doubleInvoiceTotalMoney+
         * ")，是否要存檔?"+stringAssetStatus, "訊息", JOptionPane.YES_NO_OPTION,
         * JOptionPane.WARNING_MESSAGE) ; if(ans == JOptionPane.NO_OPTION) { return
         * false ; }
         */
    ...
    if (doubleAmtC != doubleAmtD) {
      // 20180522 modified by B03812
      /*
       * //System.out.println(doubleAmtC+"---------------"+doubleAmtD) ; int ans =
       * JOptionPane.showConfirmDialog(null, "[借方金額] 不等於 [貸方金額]，是否要存檔?", "訊息",
       * JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE) ; if(ans ==
    ...
    /*
     * if(boolean2146DateCheck) { int ans = JOptionPane.showConfirmDialog(null,
     * "會計科目為 2146 或 2147 票期非 11 日是否要存檔。", "訊息", JOptionPane.YES_NO_OPTION,
     * JOptionPane.WARNING_MESSAGE) ; if(ans == JOptionPane.NO_OPTION) { return
     * false ; } }
     */
    ...
    if (!booleanSource && !"".equals(stringRetainBarCode)) {
      ...
      if (vectorDoc5M02201.size() == 0) {
        ...
        for (int intNo = 0; intNo < vectorDoc5M02201.size(); intNo++) {
          ...
          stringSql = " INSERT  INTO  Doc5M02201 (BarCode,                    BarCodeRef,               BackRetainMoney)  " + " VALUES ( '" + stringBarCode + "',  '"
              + stringBarCodeL + "',   " + stringAmt + " ) ";
          vectorSql.add(stringSql);
        }
        exeFun.doExecVectorSqlForDoc(vectorSql);
      }
      ...
      for (int intNo = 0; intNo < vectorDoc5M02201.size(); intNo++) {
        ...
        //
        stringSql = "(I.VOUCHER_YMD  =  " + stringVoucherYMDL + "  AND  " + " I.VOUCHER_FLOW_NO  =  " + stringVoucherFlowNoL + "  AND " + " I.VOUCHER_SEQ_NO  =  "
            + stringVoucherSeqNoL + " )";
        stringSql = " AND (" + stringSql + ")";
        ...
        if (stringBarCodeExcept.indexOf(stringBarCode) == -1 && exeUtil.doParseDouble(stringCanWriteAmt) < exeUtil.doParseDouble(stringAmt)) {
          messagebox("此次轉傳票請款條碼編號(" + stringBarCode + ")，欲沖銷之請款條碼編號(" + stringBarCodeL + ") 傳票可沖銷金額(" + exeUtil.getFormatNum2(stringCanWriteAmt) + ")小於本次沖銷金額("
              + exeUtil.getFormatNum2(stringAmt) + ")。");
          return false;
        }
      }
    }
  ...
  public boolean isObjectCdStopUseOK(String stringBarCode, String stringObjectCd, Doc2M010 exeFun, FargloryUtil exeUtil) throws Throwable {
    ...
    //
    vectorDoc2M010 = exeFun.getQueryDataHashtableDoc("Doc2M010", new Hashtable(), " AND  BarCode  =  '" + stringBarCode + "'  ", new Vector(), exeUtil);
    if (vectorDoc2M010.size() == 0) return false;
    hashtableDoc2M010 = (Hashtable) vectorDoc2M010.get(0);
    if (hashtableDoc2M010 == null) return false;
  ...
  public boolean isA312105BackRentSysOK(Doc2M010 exeFun, FargloryUtil exeUtil, Vector vectorRentSql) throws Throwable {
    ...
    for (int intNo = 0; intNo < retRentRemainSTemporary.length; intNo++) {
      ...
      stringSql = " UPDATE  rent_remain_temporary SET  agg_temporary  =  " + convert.FourToFive("" + doubleAggTemporaryNet, 0) + " " + " WHERE  agg_uuid  =  '" + stringAggUuid
          + "' ";
      ...
      stringSql = exeUtil.doInsertDB("REAGTLG", hashtableData, false, dbRent);
      vectorRentSql.add(stringSql);
      stringMailContent += stringSql + "<br>";
    }
    if (doubleAmtSum != 0) {
  ...
  public String[][] getRentRemainSTemporary(String stringFirmNo, String stringDeptCd, String stringFactoryNo, FargloryUtil exeUtil, talk dbRent) throws Throwable {
    ...
    stringSql = " SELECT  agg_temporary,  agg_temporary_org,  agg_uuid,  agg_id,  own_id,  " + " cas_name,           act_ago_id " + " FROM  rent_remain_temporary "
        + " WHERE  own_id  =  '" + stringFirmNo + "' " + " AND  cas_name  =  '" + stringDeptCd + "' " + " AND  own_id  =  '" + stringFactoryNo + "' "
  ...
  public double getUseAmtReagTlg(String stringAggUuid, FargloryUtil exeUtil) throws Throwable {
    ...
    for (int intNo = 0; intNo < vectorReagTlg.size(); intNo++) {
      hashtableReagTlg = (Hashtable) vectorReagTlg.get(intNo);
      if (hashtableReagTlg == null) continue;
  ...
  public double getAmtDoc5M0224(Vector vectorDoc5M0224, FargloryUtil exeUtil) throws Throwable {
    ...
    for (int intNo = 0; intNo < vectorDoc5M0224.size(); intNo++) {
      hashtableDoc5M0224 = (Hashtable) vectorDoc5M0224.get(intNo);
      if (hashtableDoc5M0224 == null) continue;
  ...
  public boolean doWriteVoucher(int intVoucherSeqNo, String stringVoucherYMDRoc, String stringVoucherFlowNo, String stringUserN, String[] arrayDoc2M014,
      String[][] retWriteVouceherData, Doc2M010 exeFun, FargloryUtil exeUtil, Vector vectorSql) throws Throwable {
    ...
    for (int intNo = 0; intNo < retWriteVouceherData.length; intNo++) {
      ...
      //
      stringSqlAnd = " (I.VOUCHER_YMD  =  " + stringVoucherYmdL + "  AND  " + " I.VOUCHER_FLOW_NO  =  " + stringVoucherFlowNoL + "  AND " + " I.VOUCHER_SEQ_NO  =  "
          + stringVoucherSeqNoL + " )";
      ...
      //
      stringSql = exeFun.getUpdateFED1013Sql(stringMoney, stringVoucherYmdL, stringVoucherFlowNoL, stringVoucherSeqNoL, stringCompanyCd, stringKind);
      vectorSql.add(stringSql);
      System.out.println("沖銷傳票(FED1013)---------" + stringSql);
      // 新增 FED1014
      stringSql = exeFun.getInsertFED1014Sql(stringVoucherYmdL, stringVoucherFlowNoL, stringVoucherSeqNoL, stringCompanyCd, stringKind, stringVoucherYMDRoc, stringVoucherFlowNo,
          "" + intVoucherSeqNo, stringMoney, " ", stringUserN);
      vectorSql.add(stringSql);
  ...
  public void doSaveDoc2M0142(Vector vectorDocSql, FargloryUtil exeUtil, Doc2M010 exeFun) throws Throwable {
    ...
    // AS_RECEIPT
    stringSql = " UPDATE    AS_RECEIPT " + " SET  RCT_UTM_DATE  =  '" + stringAcceptanceYMD + "', " + " RCT_LAST_USER  =  '" + stringLastUser + "', "
        + " RCT_LAST_DATE  =   getDate() " + // 2012-11-12 17:44:45.103
        " WHERE  RCT_COMP_ID  =  '" + arrayPurchaseData[0].trim() + "' " + " AND  PUR_ID  =  '" + arrayPurchaseData[2].trim() + "' " +
    ...
    // AS_ASSET
    stringSql = " UPDATE    AS_ASSET " + " SET  ASS_PUR_DATE  =  '" + stringAcceptanceYMD + "', " + "  ASS_LAST_USER  =  '" + stringLastUser + "', "
        + "  ASS_LAST_DATE  =  getDate() " + // 2012-11-12 17:44:45.103
        " WHERE  RCT_ID IN (SELECT  RCT_GUID  " + " FROM  AS_RECEIPT " + " WHERE  RCT_COMP_ID  =  '" + arrayPurchaseData[0].trim() + "' " + " AND  PUR_ID  =  '"
        + arrayPurchaseData[2].trim() + "' " +
        // " AND = '" +arrayPurchaseData[1].trim() +"' " + // 暫時 KindNo 不作查詢限制
        " AND  SUP_ID  =  '" + arrayPurchaseData[3].trim() + "') ";
    vectorAssetSql.add(stringSql);
  ...
  public boolean isAssetOK(boolean booleanSource, String stringBarCode, FargloryUtil exeUtil, Doc2M010 exeFun, Hashtable hashtableAsset) throws Throwable {
    ...
    boolean booleasnLastDoc = exeFun.isLastDocVoucher(booleanSource, stringBarCode, arrayPurchaseData[0].trim(), arrayPurchaseData[5].trim(), arrayPurchaseData[2].trim(),
        arrayPurchaseData[3].trim());
    //
  ...
  public boolean isVoucherCheckOK(String stringBarCode, String[] arrayPurchaseData, FargloryUtil exeUtil, Doc2M010 exeFun, Hashtable hashtableAsset) throws Throwable {
    ...
    // 請款金額- 借款沖銷
    if ("A".equals(stringTYPE)) {
      stringSql = "SELECT  DISTINCT  M17.BarCode " + " FROM  " + stringTable671 + " M17,  " + stringTable60 + "  M10 " + " WHERE  M17.BarCode  =  M10.BarCode "
          + " AND  M10.BarCode  <>  '" + stringBarCode + "' " + " AND  M10.ComNo  =  '" + stringComNo + "' " + " AND  M10.KindNo  =  '" + stringKindNo + "' "
          + " AND  M10.PurchaseNo  =  '" + stringPurchaseNo + "' " + " AND  M17.FactoryNo  =  '" + stringFactoryNo + "' " + " ORDER BY  M17.BarCode ";
    ...
    for (int intNo = 0; intNo < vectorBarCode.size(); intNo++) {
      ...
      stringSql = "SELECT  RecordNo12,  SUM(PurchaseNoTaxMoney) " + " FROM  " + stringTable171 + "   " + " WHERE  1=1 " + " AND  BarCode  =  '" + stringBarCodeL + "' "
          + " GROUP  BY  RecordNo12 ";
      retDoc6M0171 = exeFun.getTableDataDoc(stringSql);
  ...
  public String getDeptCd(String stringRecordNo12, String[] arrayPurchaseData, FargloryUtil exeUtil, Doc2M010 exeFun, Hashtable hashtableDeptCd) throws Throwable {
    ...
    if ("A".equals(stringTYPE)) {
      ...
      stringSql = " SELECT M12.InOut,  M12.DepartNo,  M12.ProjectID1,  M12.BarCode " + " FROM  Doc5M011  M11,  Doc5M0121  M12 " + " WHERE M11.BarCode  =  M12.BarCode "
          + " AND  M11.ComNo  =  '" + stringComNo + "' " + " AND  M11.ComNo  =  '" + stringKindNo + "' " + " AND  M11.DocNo  =  '" + stringPurchaseNo + "' "
          + " AND  M12.RecordNo  =  '" + stringRecordNo12 + "' ";
      ...
      //
      stringSql = " SELECT CostID " + " FROM  Doc5M012 " + " WHERE  BarCode  =  '" + retDoc3M0123[0][3].trim() + "' " + " AND  RecordNo  =  '" + stringRecordNo12 + "' ";
      ...
      stringDeptCd = exeFun.getVoucherDepartNoDoc5(stringComNo, stringComNoType, stringInOut, stringDepartNo, stringProjectID1, stringProjectID1, stringAccountNo, hashtableInOut,
          exeUtil);
    }
  ...
  public boolean isSameAsAsset(String stringFiletrDo, String[] arrayPurchaseData, String[][] retAsAsset, FargloryUtil exeUtil, Doc2M010 exeFun) throws Throwable {
    String stringBarCode = getValue("BarCode").trim();
    if ("B67283".indexOf(stringBarCode) != -1) return true;
    String stringComNo = arrayPurchaseData[0].trim();
    String stringKindNoPurchase = arrayPurchaseData[1].trim();
    String stringPurchaseNo = arrayPurchaseData[2].trim();
    String stringFactoryNo = arrayPurchaseData[3].trim();
    ...
    // 請購單之固資項次
    stringSql = " SELECT  M12.FILTER,  M12.RecordNo " + " FROM  " + stringTable11 + " M11,  " + stringTable12 + " M12 " + " WHERE  M11.BarCode  =  M12.BarCode "
        + " AND  M11.ComNo  =  '" + stringComNo + "' " + " AND  M11.DocNo  =  '" + stringPurchaseNo + "' " + " AND  M11.KindNo  =  '" + stringKindNoPurchase + "' ";
    retDoc3M012 = exeFun.getTableDataDoc(stringSql);
    ...
    // 請款金額- 請款
    stringSql = "SELECT  M17.RecordNo12,  SUM(M17.PurchaseMoney),  SUM(M17.PurchaseNoTaxMoney) " + " FROM  " + stringTable171 + " M17,  " + stringTable10 + "  M10 "
        + " WHERE  M17.BarCode  =  M10.BarCode " + " AND  M10.ComNo  =  '" + stringComNo + "' " + " AND  M17.PurchaseNo  =  '" + stringPurchaseNo + "' " + " AND  M10.KindNo  =  '"
        + stringKindNo + "' " + " AND  M17.FactoryNo  =  '" + stringFactoryNo + "' " + " GROUP BY  M17.RecordNo12 ";
    retDoc2M0171 = exeFun.getTableDataDoc(stringSql);
    ...
    // 請款金額- 借款沖銷
    if ("A".equals(stringTYPE)) {
      stringSql = "SELECT  M17.RecordNo12,  M17.PurchaseMoney,  M17.PurchaseNoTaxMoney " + " FROM  " + stringTable671 + " M17,  " + stringTable60 + "  M10 "
          + " WHERE  M17.BarCode  =  M10.BarCode " + " AND  M10.ComNo  =  '" + stringComNo + "' " + " AND  M10.KindNo  =  '" + stringKindNo + "' " + " AND  M10.PurchaseNo  =  '"
          + stringPurchaseNo + "' " + " AND  M10.FactoryNo  =  '" + stringFactoryNo + "' ";
    ...
    for (int intNo = 0; intNo < retAsAsset.length; intNo++) {
      ...
      for (int intNoL = 0; intNoL < retDoc2M0171.length; intNoL++) {
        ...
        if (stringRecordNo12Sum.indexOf("," + stringRecordNo12 + ",") == -1) {
          // doublePurchaseMoneySumDoc += exeUtil.doParseDouble(stringPurchaseMoney) ;
          // doublePurchaseNoTaxMoneySumDoc +=
          // exeUtil.doParseDouble(stringPurchaseNoTaxMoney) ;
          // System.out.println(intNo+"
          // 1檢核金額合計，請款系統("+stringPurchaseMoney+")。--------------------") ;
          continue;
        }
        //
      ...
      for (int intNoL = 0; intNoL < retDoc6M0171.length; intNoL++) {
        ...
        if (stringRecordNo12Sum.indexOf("," + stringRecordNo12 + ",") == -1) {
          ...
          // exeUtil.doParseDouble(stringPurchaseNoTaxMoney) ;
          // System.out.println(intNo+"
          // 2檢核金額合計，請款系統("+stringPurchaseMoney+")。--------------------") ;
          continue;
        }
        //
        doublePurchaseMoneyDoc += exeUtil.doParseDouble(stringPurchaseMoney);
  ...
  public boolean isApplyTypeD(String[] arrayPurchaseData, Doc2M010 exeFun) throws Throwable {
    ...
    stringSql = " SELECT  M12.FILTER " + " FROM  " + stringTable12 + " M12,  " + stringTable11 + " M11 " + " WHERE  M11.BarCode  =  M12.BarCode "
        + " AND  ISNULL(FILTER,  '')  <>  ''  " + " AND  M11.ComNo   =  '" + stringComNo + "' " + " AND  M11.KindNo  =  '" + stringKindNo + "' " + " AND  M11.DocNo   =  '"
        + stringPurchaseNo + "' ";
  ...
  public String[][] getAsAsset(String[] arrayPurchaseData, Doc2M010 exeFun, talk dbAsset) throws Throwable {
    ...
    for (int intNo = 0; intNo < retTableData.length; intNo++) {
      ...
      arrayTemp = (String[]) vectorData.get(intPos);
      if (arrayTemp == null) continue;
  ...
  public String[][] getDoc5M020(String stringComNo, String stringFactoryNo, Vector vectorPurchaseNo, Doc.Doc2M010 exeFun) throws Throwable {
    ...
    stringSql = "SELECT  M10.BarCode,  M10.RetainMoney " + " FROM  " + stringTable17 + " M17,  " + stringTable10 + " M10 " + " WHERE  M17.BarCode  =  M10.BarCode "
        + " AND  M10.ComNo  =  '" + stringComNo + "' " + " AND  M17.FactoryNo  =  '" + stringFactoryNo + "' " + " AND  M17.PurchaseNo  IN (" + stringPurchaseNoSql + ") "
        + " AND  M10.BarCode  IN  (SELECT  BarCode  FROM  Doc2M014  WHERE  STATUS_CD  = 'Z') " + " ORDER BY  EDateTime ";
  ...
  public Vector doInsertDiscount(String[][] retDoc2M014, double doubleDiscountTotalMoney, Vector vectorError, Doc.Doc2M010 exeFun, FargloryUtil exeUtil) throws Throwable {
    ...
    for (int intNo = 0; intNo < 100; intNo++) {
      if (intNo < retDoc2M011.length) {
        ...
        exeFun.doInsertDoc2M016(stringDiscountNo, stringDiscountDate, stringBarCode, stringInvoiceNo, stringCompanyCdO, stringDescription, stringDepartNo, doubleDiscountMoney);
  ...
  public void doInsertDiscount(boolean booleanSource, String[] retDoc2M014, Doc2M010 exeFun, FargloryUtil exeUtil) throws Throwable {
    ...
    stringDiscountNo = exeFun.getDiscountNo(stringCompanyCdO, stringDepartNo, stringDiscountDate);
    stringDescription = getDescription(stringBarCode, stringDescription, exeFun, exeUtil);
    exeFun.doInsertDoc2M016(stringDiscountNo, stringDiscountDate, stringBarCode, stringInvoiceNo, stringCompanyCdO, stringDescription, stringDepartNo, stringDiscountTotalMoney,
  ...
  public void doInsertDiscountNoTax(boolean booleanSource, String stringAmt, String[] retDoc2M014, Doc.Doc2M010 exeFun, FargloryUtil exeUtil) throws Throwable {
    ...
    String stringDescription = retDoc2M014[23].trim();
    String stringInvoiceNo = booleanSource ? exeFun.getDoc2M011(stringBarCode)[0][3].trim() : exeFun.getDoc5M021(stringBarCode)[0][3].trim();//
    String stringDepartNo = (booleanSource ? exeFun.getDoc2M010(stringBarCode) : exeFun.getDoc5M020(stringBarCode))[0][12].trim();
    String stringDiscountNo = "";
    //
    stringDiscountNo = exeFun.getDiscountNo(stringCompanyCdO, stringDepartNo, stringDiscountDate);
  ...
  public void doDoc2M010etc(boolean booleanSource, Doc.Doc2M010 exeFun, FargloryUtil exeUtil) throws Throwable {
    ...
    retDoc = booleanSource ? exeFun.getDoc2M011(stringBarCode) : exeFun.getDoc5M021(stringBarCode); // 取得發票資料，Doc2M011
    if (retDoc.length == 0) {
      // 0 FactoryNo 1 ReceiptKind 2 ReceiptDate 3 ReceiptMoney 4 ReceiptTax
      // 5 ReceiptTotalMoney 6 ReceiptTaxType
      retDoc = booleanSource ? exeFun.getDoc2M013(stringBarCode) : exeFun.getDoc5M023(stringBarCode); // 取得扣繳資料，Doc2M013
    ...
    // Doc3M010
    // 付款條件更新
    String[][] retDoc2M017 = booleanSource ? exeFun.getDoc2M017(stringBarCode) : exeFun.getDoc5M027(stringBarCode);
    for (int intNo = 0; intNo < retDoc2M017.length; intNo++) {
      ...
      if (booleanSource && !"".equals(stringPurchaseNo1) && !"".equals(stringPurchaseNo2) && !"".equals(stringPurchaseNo3)) {
        exeFun.doUpdateForDoc3M010(stringPayCondition1, stringPayCondition2, "N", stringComNo, stringPurchaseNo1, stringPurchaseNo2, stringPurchaseNo3, stringPurchaseNo4);
      }
    }
    ...
    for (int intRowNo = 0; intRowNo < jtable1.getRowCount(); intRowNo++) {
      ...
      if ("D".equals(stringDbCrCd)) {
        if ("A".equals(stringRowType.substring(0, 1))) {
          ...
          if (!"".equals(stringDescription5)) stringSql = exeFun.getUpdateDoc2M011SqlUnion(stringBarCode, stringRecordNo, stringDescription5, stringDescription1,
              stringDescription2, stringDescription3, stringAMT, stringDescription4, stringTaxType, booleanSource ? "Doc2M011" : "Doc5M021");
            ...
            for (int intNo = 0; intNo < jtable1.getRowCount(); intNo++) {
              ...
              if ("H".equals(stringRowTypeL)) {
                if (stringRecordNoL.equals(stringRecordNoDoc2M015)) {
                  // System.out.println(intRowNo+"----"+intNo+"------費用對應的 折讓未稅金額
                  // -------"+stringDiscountMoneyL) ;
                  ...
                  // System.out.println(intRowNo+"----"+intNo+"------費用對應的 特別折讓金額
                  // -------"+stringDiscountTotalMoney) ;
                  //
                  stringDiscountTotalMoney = "" + (exeUtil.doParseDouble(stringDiscountTotalMoney) / (doubleTaxRate + 1)); // 變成未稅
                  // System.out.println(intRowNo+"----"+intNo+"------費用對應的 特別未稅折讓金額
                  // -------"+stringDiscountTotalMoney) ;
                  stringDiscountMoneyL = convert.FourToFive("" + (exeUtil.doParseDouble(stringDiscountMoneyL) - exeUtil.doParseDouble(stringDiscountTotalMoney)), 0);
                  break;
                }
        ...
        if ("G".equals(stringRowType.substring(0, 1)) && !(intAccountCount > 1)) {
          ...
          stringSql = exeFun.getUpdateDoc2M013SqlUnion(stringAMT, stringReceiptDate, stringTotalReceiptMoney, stringReceiptMoney, stringReceiptTaxType, stringBarCode,
              stringRecordNo, booleanSource ? "Doc2M013" : "Doc5M023");
          // System.out.println(intRowNo+"扣繳
          // -------------------------------------"+stringSql) ;
        }
        if ("H".equals(stringRowType.substring(0, 1))) {
          // 折讓
          // 須扣除特殊折讓
          /*
           * String stringDiscountTotalMoney = exeFun.getSpecialDiscountTotalMoneyUnion
           * (booleanSource?"Doc2M015":"Doc5M025", stringBarCode, stringRecordNo) ; //
           * stringDescription4 =
           * convert.FourToFive(""+(exeUtil.doParseDouble(stringDescription4) -
           * exeUtil.doParseDouble(stringDiscountTotalMoney)), 0) ;
           * if(exeUtil.doParseDouble(stringDescription4) >= 0) {
           * exeFun.doUpdateDiscountMoneyForDoc2M015Union(booleanSource?"Doc2M015":
           * "Doc5M025", stringDescription4, stringBarCode, stringRecordNo) ; }
           */
        }
