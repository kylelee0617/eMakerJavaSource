...
public class Doc3R011_Print1 extends bproc {
  ...
  public void doExcel(String stringThisFunction, FargloryUtil exeUtil, talk dbDoc, talk dbFE3D, talk dbFED1, talk dbAsset) throws Throwable {
    ...
    if (!(new File(stringFilePath)).exists()) {
      // stringFilePath =
      // "https://emaker11.farglory.com.tw/servlet/baServer3?step=6?filename="+stringFilePath
      // ;
    ...
    /*
     * //2018/03/08 - 增加列印法令遵循檢核 exeExcel.putDataIntoExcel(0, 29,
     * "法令遵循檢核：1.利害關係人交易:□否□是，檢附:□利害關係人查詢記錄\n 　　　　　　　2.委外作業事項:　□否□是，檢附:□廠商評鑑表",
     * objectSheet1) ;
     */

    if (stringThisFunction.indexOf("請購申請書-簽核彙總") == -1) messagebox("列印完成。\n(改成A4雷射列印。)");
    // 刪除
    // stringSheetName = "A".equals(stringPrintType) ? "sheet2" : "sheet1" ;
    // objectSheet1 = Dispatch.call(objectSheets, "Item",
    // stringSheetName).toDispatch() ;
    // exeExcel.doDeleteSheet(objectSheet1 ) ;
    // 釋放 Excel 物件
  ...
  public void doPrintDoc3M011(boolean booleanSource, String stringPrintType, Vector vectorDoc3M011, Vector vectorDoc3M012, Dispatch objectSheet1, FargloryExcel exeExcel,
    ...
    // exeExcel.putDataIntoExcel(0, 2, "單位（案別）："+stringDepartName, objectSheet1) ;
    // exeExcel.putDataIntoExcel(0, 2, "單位（案別）："+stringDepartNo+"
    // "+stringDepartName, objectSheet1) ;
    // 申購名稱
