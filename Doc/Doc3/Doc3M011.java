...
public class Doc3M011 extends bTransaction {
  public boolean action(String value) throws Throwable {
    ...
    String stringUndergoWirteCheck = "" + get("Doc3M011_UNDERGO_CHECK");
    put("Doc3M011_UNDERGO_CHECK", "null");
    put("Doc3M011_UNDERGO_CHECK_L", stringUndergoWirteCheck);
    ...
    try {
      ...
      if ("SYS".equals(getUser()) && !"刪除".equals(value)) {
         ...
         */
        /*
         * if("刪除".equals(value.trim( ))) { exeFun.doDeleteDoc1M040(stringBarCode) ;
      ...
      if (!isFlowCheckOK(value.trim(), exeFun, exeUtil)) {
      ...
      if ("新增".equals(value.trim()) && stringFunctionName.indexOf("承辦") != -1) {
        ...
        // if(stringFunctionName.indexOf("--")==-1) setValue("BarCode",
        // exeFun.getMaxBarCode("Z")) ;
        // }
      }
      // 簽核流程記錄
      ...
      exeFun.doInsertForDoc3M011History(stringID, datetime.getTime("YYYY/mm/dd h:m:s"), stringUser, stringDeptCd,
          getFunctionName() + " " + value + "---" + stringBarCodeE + "---" + stringBarCodeOldE, stringClassNameList + "---" + stringDocNo, stringComNo, true);
    ...
    getButton("ButtonTable10View").doClick();
    if (",B3018,".indexOf("," + getUser().toUpperCase() + ",") != -1) {
      messagebox("TEST");
      return false;
    }
    return true;
  ...
  public void doInsertDoc5M0182(FargloryUtil exeUtil, Doc2M010 exeFun) throws Throwable {
    ...
    if (stringFlow.indexOf("採購") != -1) {
      // stringUndergoName = ("Y".equals(stringUndergoWirteCheck))?"採購室承辦-送呈" :
      // "採購室承辦" ;
    ...
    stringSql = exeFun.doInsertDBDoc("Doc5M0182", hashtableData, false, exeUtil);
  ...
  public void doKindNoD(FargloryUtil exeUtil, Doc2M010 exeFun) throws Throwable {
    ...
    /*
     * 2016-07-18 修正 17-6 一般類請購單(1000萬以上) 17-5 一般類請購單(200萬~1000萬) 17-4
     * 一般類請購單(200萬以下)
     */
  ...
  public boolean isCheckDoc3M080(Doc.Doc2M010 exeFun, FargloryUtil exeUtil) throws Throwable {
    ...
    if (retDoc8M010 == null || retDoc8M010.length == 0 || retDoc8M010[0][0].trim().length() == 0 || "0".equals(retDoc8M010[0][0].trim())) {
      ...
      String stringSql = "";
      // 存在
      stringSql = " UPDATE  Doc8M010  SET  ClassName  =  (SELECT  ClassName  FROM  Doc3M012  WHERE  RecordNo  =  '1'  AND  ID  =  '" + convert.ToSql(stringID)
          + "'),  FactoryNo   =  (SELECT  FactoryNo  FROM  Doc3M012  WHERE  RecordNo  =  '1'   AND  ID  =  '" + convert.ToSql(stringID) + "'), "
          + " PurchaseEmployeeNo  =  (SELECT  PurchaseEmployeeNo  FROM  Doc3M011  WHERE  ID  =  '" + convert.ToSql(stringID) + "'), "
          + " BudgetMoney  =  (SELECT  SUM(ApplyMoney)  FROM   Doc3M012  WHERE  ID  =  '" + convert.ToSql(stringID) + "'), "
          + " RealMoney       =  (SELECT  SUM(PurchaseMoney)  FROM  Doc3M012  WHERE  ID  =  '" + convert.ToSql(stringID) + "'), "
          + " DifferenceMoney  =  (SELECT  ISNULL(SUM(ApplyMoney),0) - ISNULL(SUM(PurchaseMoney),0)  FROM  Doc3M012  WHERE  ID  =  '" + convert.ToSql(stringID) + "'), "
          + " DifferenceRate  =  (CASE (SELECT  SUM(ApplyMoney) FROM Doc3M012  WHERE  ID  =  '" + convert.ToSql(stringID) + "')  WHEN  0   THEN  0  "
          + " ELSE (SELECT  ((ISNULL(SUM(ApplyMoney),0) - ISNULL(SUM(PurchaseMoney),0)) /  ISNULL(SUM(ApplyMoney),1) * 100)  FROM  Doc3M012  WHERE  ID  =  '"
          + convert.ToSql(stringID) + "')  end)  WHERE  ComNo  =  '" + convert.ToSql(stringComNo) + "'  AND  BarCode  =  '" + convert.ToSql(stringBarCode) + "' "
  ...
  public boolean isFlowCheckOK(String value, Doc.Doc2M010 exeFun, FargloryUtil exeUtil) throws Throwable {
    ...
    // 20201110 Kyle : 想改為取消....再看看
    // 後續流程已使用，不可異動資料庫
    // 請款
    String stringDocNo = getValue("DocNo1").trim() + getValue("DocNo2").trim() + getValue("DocNo3").trim();
    String[][] retDoc5M027 = exeFun.getTableDataDoc("SELECT  M10.BarCode,  M10.UNDERGO_WRITE  FROM  Doc2M017 M17,  Doc2M010  M10   WHERE  M17.BarCode  =  M10.BarCode "
        + " AND  M17.PurchaseNo  =  '" + stringDocNo + "'  AND  M10.ComNo  =  '" + stringComNo + "' ");
    ...
    // Doc5M030 借款沖銷
    String[][] retDoc5M030 = exeFun
        .getTableDataDoc("SELECT  BarCode,  UNDERGO_WRITE  FROM  Doc6M010  WHERE  PurchaseNo  =  '" + stringDocNo + "'  AND  ComNo  =  '" + stringComNo + "' ");
    ...
    if (stringFlow.indexOf("承辦") != -1 && stringFlow.indexOf("--") == -1) {
      ...
      /*
       * if("R".equals(stringUnderGoWrite)) { JOptionPane.showMessageDialog(null,
       * "採購退件資料，不可執行 [修改] [刪除] 功能。1", "訊息", JOptionPane.ERROR_MESSAGE) ; return false
       * ; }
       */
    ...
    if (stringFlow.indexOf("採購") != -1) {
      ...
      if (booleanAssetDate && ",P83219,E35990,J29564,P83218,".indexOf("," + stringBarCodeOld + ",") == -1) {
        ...
        /*
         * // 固資 if("D".equals(stringApply)) { String[][] retDoc1M040 =
         * exeFun.getTableDataDoc("SELECT  Barcode  FROM  Doc1M040 " +
         * " WHERE  Barcode  =  '"+stringBarCodeOld+"' "+
         * " AND  DepartNo  LIKE  '023%' "+ " AND  DocStatus  =  '4' ") ;
         * if(retDoc1M040.length == 0) { messagebox("[固定資產]請購單，[人總] 公文追蹤尚未出文，不可異動資料庫。")
         * ; return false ; } }
         */
      }
    }
  ...
  public boolean isFlowCheckOKNew(String stringUnderGoWrite, Vector vectorUser, Doc2M010 exeFun, FargloryUtil exeUtil) throws Throwable {
    ...
    if (stringFlow.indexOf("承辦") != -1 && stringFlow.indexOf("--") == -1) {
      ...
      /*
       * if("R".equals(stringUnderGoWrite)) { messagebox("採購退件資料，不可執行 [修改] [刪除] 功能。")
       * ; return false ; }
       */
      // A 已送呈 不可執行 [修改] [刪除] 功能。
    ...
    if (stringFlow.indexOf("採購") != -1) {
      ...
      /*
       * if("D".equals(stringApply)) { String[][] retDoc1M040 =
       * exeFun.getTableDataDoc("SELECT  Barcode  FROM  Doc1M040 " +
       * " WHERE  Barcode  =  '"+stringBarCodeOld+"' "+
       * " AND  DepartNo  LIKE  '023%' "+ " AND  DocStatus  =  '4' ") ;
       * if(retDoc1M040.length == 0) { messagebox("[固定資產]請購單，[人總] 公文追蹤尚未出文，不可異動資料庫。")
  ...
  public void setFlowDataNew(FargloryUtil exeUtil, Doc2M010 exeFun) throws Throwable {
    ...
    // 20180426 無採購科主管流程
    /*
     * // 採購科主管處理 doSetPurchaseSupervisor(booleanUndergoWriteCheck, stringDeptCd,
     * stringTodayTime, exeUtil, exeFun) ;
     */
  ...
  public void doSetPurchaseSupervisor(boolean booleanUndergoWriteCheck, String stringDeptCd, String stringTodayTime, FargloryUtil exeUtil, Doc2M010 exeFun) throws Throwable {
    ...
    // 承辦人員之科主管
    String stringPurchaseSupervisor = exeFun.getNameUnionFE3D("CENSOR_EMP_NO", "FE3D110", " AND  DEPT_CD  =  '" + stringDeptCd + "' AND  ITEM  =  '2' ", new Hashtable(), exeUtil);
    if ("".equals(stringPurchaseSupervisor)) return;
    ...
    hashtableData.put("Descript", "");
    String stringSql = exeFun.doInsertDBDoc("Doc5M0181", hashtableData, false, exeUtil);
    System.out.println("" + stringSql);
    addToTransaction(stringSql);
  ...
  public void doSetBackFlowData(String stringTodayTime, FargloryUtil exeUtil, Doc2M010 exeFun) throws Throwable {
    // System.out.println("退件之承辦簽核時，直接簽核至該退件者前一關卡
    // doSetBackFlowData------------------------------------------S") ;
    if (getFunctionName().indexOf("業管") == -1) return; // 非業管，退件不處理
    // 本請購單之前有退件記錄
    String stringBarCode = getValue("BarCode").trim();
    Vector vectorDoc5M0182 = exeFun.getQueryDataHashtableDoc("Doc5M0182", new Hashtable(), " AND BarCode  =  '" + stringBarCode + "' ORDER BY  CheckDate DESC ", new Vector(),
        exeUtil);
    if (vectorDoc5M0182.size() == 0) return;
    String stringCheckEmployeeNo = "";
    String stringUndergoName = "";
    String stringUndergoWrite = "";
    ...
    for (int intNo = 0; intNo < vectorDoc5M0182.size(); intNo++) {
      ...
      stringUndergoWrite = exeUtil.getVectorFieldValue(vectorDoc5M0182, intNo, "UNDERGO_WRITE");
      if ("null".equals(stringUndergoWrite)) stringUndergoWrite = "";
      ...
      // System.out.println(intNo+"-----簽核關卡代碼 vs
      // 簽核人員-------------------------------------1") ;
      //
  ...
  public boolean isBatchCheckOK(String stringFunction, Doc.Doc2M010 exeFun, Farglory.util.FargloryUtil exeUtil) throws Throwable {
    ...
    String stringUndergoWirteCheck = "" + get("Doc3M011_UNDERGO_CHECK_L");
    boolean booleanUndergoWriteCheck = stringFlow.indexOf("採購") == -1 || "Y".equals(stringUndergoWirteCheck);
    // 代銷合約
    ...
    if (booleanUndergoWriteCheck && stringSpecBudget.indexOf("," + stringDocNo1 + ",") != -1 && exeFun.getTableDataDoc(
        "SELECT  EmployeeNo  FROM  Doc3M011_EmployeeNo  WHERE  FunctionType  =  '" + stringFunctionType + "'  AND  EmployeeNo  =  '" + stringEmployeeNo + "' ").length <= 0) {
    ...
    /*
     * if(!"A".equals(exeFun.getUseType(stringComNo, ""))) { messagebox("公司 " +
     * stringComNo +
     * "("+exeFun.getCompanyName(stringComNo)+") 不允許使用。\n(有問題請洽 [財務室])") ;
     * getcLabel("ComNo").requestFocus( ) ; return false ; }
     */
  ...
  public boolean isTable1CheckOK(boolean booleanUndergoWriteCheck, Doc2M010 exeFun, FargloryUtil exeUtil) throws Throwable {
    ...
    boolean booleanAssetDate = true;// !"".equals(stringAssetDate) && !"null".equals(stringAssetDate) &&
                                    // stringToday.compareTo(stringAssetDate)>=0 ;
    //
    ...
    String[][] retDoc2M0401 = exeFun.getDoc2M0401("", "U", "");
    if (retDoc2M0401.length > 0) stringFiletrDo = retDoc2M0401[0][2].trim(); // Remark 固資加工
    ...
    if (vecrorCostIDTypeO == null) vecrorCostIDTypeO = new Vector(); // 固資請款代碼
    ...
    for (int intNo = 0; intNo < jtable1.getRowCount(); intNo++) {
      ...
      if (!"D".equals(stringApplyType)) {
        ...
        if (stringBarCode.startsWith("S")) {
          ...
          stringCostID = exeUtil.getVectorFieldValue(vectorDoc2M022, 0, "CostID");
          setValueAt("Table1", stringCostID, intNo, "CostID");
          stringCostID1 = exeUtil.getVectorFieldValue(vectorDoc2M022, 0, "CostID1");
          setValueAt("Table1", stringCostID1, intNo, "CostID1");
          stringCostID2 = exeUtil.getVectorFieldValue(vectorDoc2M022, 0, "CostID2");
          setValueAt("Table1", stringCostID2, intNo, "CostID2");
        }
         ...
         * ""+vecrorCostIDTypeO.get(intNoL) ; } stringTemp = "第 " + (intNo+1) +
         * " 列 之 非 [固定資產] 的 請款代碼 不能輸入"+stringTemp+"。" ; return
         * retTable1Message(stringTemp, intNo, false, exeUtil) ; }
         */
        //
        ...
        // 20180309 移除固定資產限制
        /*
         * if("".equals(stringFILTER)) { if(booleanUndergoWriteCheck) { stringTemp =
         * "第 " + (intNo+1) + " 列 之 [固資代碼] 不可為空白。" ; return retTable1Message(stringTemp,
         * intNo, false, exeUtil) ; } } else { vectorAsAssetFilter =
         * exeUtil.getQueryDataHashtable("AS_ASSET_FILTER", new Hashtable(),
         * " AND  FILTER  = '"+stringFILTER+"' ", dbAsset) ; if(booleanUndergoWriteCheck
         * && vectorAsAssetFilter.size() == 0) { stringTemp = "第 " + (intNo+1) +
         * " 列 之 [固資代碼] 不存在資料庫中。" ; return retTable1Message(stringTemp, intNo, false,
         * exeUtil) ; } // 固資之請款代碼 依第一個案別分攤 作 分類，至 Doc2M0201 取值(略) 二層變一層時，不允許修改請款代碼 //
         * 固資計算 if(!stringFILTER.equals(stringFiletrDo)) { // 固資-請款代碼計算 intAssetCount =
         * exeUtil.doParseInteger(""+hashtableFactoryNoAsset.get(stringFactoryNo+"A")) +
         * 1 ; hashtableFactoryNoAsset.put(stringFactoryNo+"A", ""+intAssetCount) ; // }
         * else { // 固資-加工-請款代碼計算 intAssetCount =
         * exeUtil.doParseInteger(""+hashtableFactoryNoAsset.get(stringFactoryNo+"B")) +
         * 1 ; hashtableFactoryNoAsset.put(stringFactoryNo+"B", ""+intAssetCount) ; } }
         */
      }
      // 通路代碼 檢核
      if (!"".equals(stringSSMediaID)) {
        ...
        if (!stringCostIDDetail.startsWith("78")) {
          if (getFunctionName().indexOf("採購") != -1) {
            ...
            /*
             * if(booleanUndergoWriteCheck && stringDocNo1.indexOf("333")==-1 &&
             * "782".equals(stringCostID+stringCostID1)) { stringTemp = "第 " + (intNo+1) +
             * " 列 之 POP 請款代碼 活動代碼 不可為空白。" ; return retTable1Message(stringTemp, intNo,
             * false, exeUtil) ; }
             */
          }
        }
      ...
      // 統購 採購
      /*
       * if((booleanApplyF || !booleanUser) &&
       * vectorFactoryNoCostID.indexOf(stringCostIDDetail+"-"+stringFactoryNo) != -1)
       * { stringTemp = "第 " + (intNo+1) + " 列 之 [統一編號][分類工料代碼] 重複。" ; return
       * retTable1Message(stringTemp, intNo, false, exeUtil) ; }
       */
      // 請購數量 BudgetNum
      ...
      if (booleanUser) {
        ...
        if (exeUtil.doParseDouble(stringPurchaseMoney) != exeUtil.doParseDouble(stringPurchaseMoneyCompute)) {
          int ans = JOptionPane.showConfirmDialog(null, "第 " + (intNo + 1) + " 列 之 [計算金額] 不等於 [請購金額]，如要繼續請按 [是]。", "請選擇?", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
      ...
      if (booleanUndergoWriteCheck && vectorTable2FactoryNo.indexOf(stringFactoryNo) == -1
          && (vectorTable2FactoryNo.size() > 0 || "C".equals(stringUnderGoWrite) || "Y".equals(stringUnderGoWrite))) {
       ...
       * stringTemp = "第 " + (intNo+1) + " 列 之 [規格] 不可為空白。" ; return
       * retTable1Message(stringTemp, intNo, false, exeUtil) ; }
       */
    ...
    // 20180309 不檢查固定資產 By B03812
    /*
     * if(booleanUndergoWriteCheck && booleanAssetDate &&
     * stringFlow.indexOf("採購")!=-1 && "D".equals(stringApplyType) ){ for(int
     * intNo=0 ; intNo<vectorFactoryNo.size() ; intNo++) { stringFactoryNo =
     * ""+vectorFactoryNo.get(intNo) ; intAssetCount =
     * exeUtil.doParseInteger(""+hashtableFactoryNoAsset.get(stringFactoryNo+"A")) ;
     * if(intAssetCount == 0) { stringTemp =
     * "廠商("+stringFactoryNo+") 固定資產請購單之固資項目 至少 一列。" ; return
     * retTable1Message(stringTemp, -1, false, exeUtil) ; } intAssetCount =
     * exeUtil.doParseInteger(""+hashtableFactoryNoAsset.get(stringFactoryNo+"B")) ;
     * if(intAssetCount > 1) { stringTemp =
     * "廠商("+stringFactoryNo+") 固定資產請購單之非固資項目 僅 能一列。" ; return
     * retTable1Message(stringTemp, -1, false, exeUtil) ; } } }
     */
  ...
  public double getNumDoc3M012(String stringRecordNoDoc3M017, Hashtable hashtableDoc3M017, FargloryUtil exeUtil, Doc2M010 exeFun) throws Throwable {
    ...
    //
    stringSql = " SELECT  M12.ActualNum  FROM  Doc3M011 M11,  Doc3M012 M12  WHERE  M11.BarCode  =  M12.BarCode  AND  M11.UNDERGO_WRITE  <>  'X'  AND  M11.BarCode  <>  '"
        + stringBarCode + "'  AND  M11.CDate  BETWEEN  '" + stringDateStart + "'  AND  '" + stringDateEnd + "'  AND  M12.CostID  =  '" + stringCostID + "'  AND  M12.CostID1  =  '"
        + stringCostID1 + "'  AND  M12.RecordNoDoc3M017  =  " + stringRecordNoDoc3M017 + " ";
  ...
  public Hashtable getDoc3M016(String stringCostID, String stringCostID1, String stringDateStart, String stringDateEnd, FargloryUtil exeUtil, Doc2M010 exeFun) throws Throwable {
    ...
    stringSqlAnd = " AND  ( (DateStart  >=  '" + stringDateStart + "'  AND  DateStart  <=  '" + stringDateEnd + "')  OR  (DateEnd   >=  '" + stringDateStart
        + "'  AND  DateEnd   <=  '" + stringDateEnd + "')  OR  (DateStart  <=  '" + stringDateStart + "'  AND  DateEnd   >=  '" + stringDateEnd + "')  OR  (DateStart  >=  '"
        + stringDateStart + "'  AND  DateEnd   <=  '" + stringDateEnd + "')) ";
  ...
  public boolean isTable2CheckOK(boolean booleanUndergoWriteCheck, Doc.Doc2M010 exeFun, FargloryUtil exeUtil) throws Throwable {
    ...
    for (int intNo = 0; intNo < jtable2.getRowCount(); intNo++) {
      ...
      // 期數 MonthNum
      /*
       * if("".equals(stringMonthNum)) { JOptionPane.showMessageDialog(null, "第 " +
       * (intNo+1) + " 列 之 [期數] 不可為空白。", "訊息", JOptionPane.ERROR_MESSAGE) ;
       * jtabbedPane1.setSelectedIndex(intTablePanel) ; setFocus("Table2", intNo,
       * "MonthNum") ; return false ; }
       */
    }
    ...
    for (int intNo = 0; intNo < vectorFactoryNo.size(); intNo++) {
      ...
      /*
       * if(doublePercentRateSum != 100) { JOptionPane.showMessageDialog(null,
       * "[廠商] 為 " + stringFactoryNo + " 之 [比例] 和不為 100 %。", "訊息",
       * JOptionPane.ERROR_MESSAGE) ; jtabbedPane1.setSelectedIndex(intTablePanel) ;
       * return false ; }
       */
      ...
      if (doublePurchaseMoneySum != doublePurchaseMoneySum2) {
        JOptionPane.showMessageDialog(null, "[廠商] 為 " + stringFactoryNo + " 之 [金額] 總和 (" + convert.FourToFive("" + doublePurchaseMoneySum, 0) + ") 與相對應 [請購資訊] 總和 ("
            + convert.FourToFive("" + doublePurchaseMoneySum2, 0) + ") 不同 。", "訊息", JOptionPane.ERROR_MESSAGE);
        jtabbedPane1.setSelectedIndex(intTablePanel);
        return false;
      }
    }
  ...
  public boolean isTable3CheckOK(boolean booleanUndergoWriteCheck, String stringFunction, Farglory.util.FargloryUtil exeUtil, Doc.Doc2M010 exeFun) throws Throwable {
    ...
    if (!"".equals(stringActionNo) && vectorProject.size() == 0) {
      ...
      if (retDoc3M040.length == 0) {
        ...
        if (!"".equals(stringTemp)) {
          ...
          /*
           * switch(stringTemp.charAt(0)) { case 'A' : stringTemp = "創文" ; break ; case
           * 'B' : stringTemp = "主辦人" ; break ; case 'C' : stringTemp = "審核" ; break ;
    ...
    boolean booleanRealMoney = stringFlow.indexOf("採購") != -1; // ("C,Y,".indexOf(stringUndergoWrite)!=-1) || ("S".equals(stringUndergoWrite)
                                                               // && "F".equals(stringApplyType)) ;
    ...
    String stringSqlAnd807 = " AND  FunctionType LIKE  '%2%'  AND  (DateStart='9999/99/99'  OR  DateStart<='" + stringCDate + "' )"
        + " AND  (DateEnd='9999/99/99'    OR  DateEnd>='" + stringCDate + "' ) AND  ComNo  IN ('ALL',  '" + stringComNo + "' )";
    for (int intNo = 0; intNo < jtable3.getRowCount(); intNo++) {
      ...
      //
      vectorDoc2M0201 = exeFun.getQueryDataHashtableDoc("Doc2M0201", new Hashtable(),
          stringSqlAnd807 + " AND  CostID  =  '" + stringCostID + "'  AND  CostID1  =  '" + stringCostID1 + "' ", new Vector(), exeUtil);
      ...
      // 人壽案別檢查
      /*
       * if("Z6".equals(stringComNo) && !"".equals(stringProjectID1)&&
       * "H56A,H85A".indexOf(stringProjectID1)!=-1) { messagebox("第 " +(intNo+1)
       * +" 列 [遠雄房屋] 不允許使用 H56A、H85A 之 案別！，不允許異動資料庫。\n(有問題請洽 [財務室])") ;
       * jtabbedPane1.setSelectedIndex(intTablePanel) ;
       * jtable3.setRowSelectionInterval(intNo, intNo) ; return false ; }
       */
      ...
      //
      /*
       * if(exeUtil.doParseDouble(stringRealMoney) == 0) { if(booleanUser){
       * JOptionPane.showMessageDialog(null, "第 " + (intNo+1) + " 列 之 [金額]  不可為 0。",
       * "訊息", JOptionPane.ERROR_MESSAGE) ;
       * jtabbedPane1.setSelectedIndex(intTablePanel) ;
       * jtable3.setRowSelectionInterval(intNo, intNo) ; return false ; } }
       */
      // 重複檢核
      ...
      booleanTemp = !("001".equals(stringCostID + stringCostID1)) && !"".equals(stringProjectID1Use) && !"".equals(stringType)
      ...
      if (!"".equals(stringProjectID1Use) && vectorProjectID1BudgetID.indexOf(stringProjectID1Use + "-" + stringBudgetID) == -1
          && vectorProjectID1NoUseBudget.indexOf(stringDepartNo + "---" + stringProjectID + "---" + stringProjectID1) == -1)
        vectorProjectID1BudgetID.add(stringProjectID1Use + "-" + stringBudgetID);
      // 總金額 須等於 請購資訊總金額 RealMoney
      doubleRealMoney = exeUtil.doParseDouble(stringRealMoney);
      /*
       * if(booleanUser && doubleRealMoney == 0) { JOptionPane.showMessageDialog(null,
       * "第 " + (intNo+1) + " 列 之 [總金額] 不可為 0。", "訊息", JOptionPane.ERROR_MESSAGE) ;
       * jtabbedPane1.setSelectedIndex(intTablePanel) ; setFocus("Table3", intNo,
       * "RealMoney") ; return false ; }
       */
      // 資料處理
      ...
      for (int intNo = 0; intNo < vectorProject.size(); intNo++) {
        ...
        if (doubleCheckMoney + 100 < doubleUseMoney) {
          JOptionPane.showMessageDialog(null,
              "案別(" + stringKeyL + ") 已使用金額(" + exeUtil.getFormatNum2("" + doubleUseMoney) + ")超過專案可用金額(" + exeUtil.getFormatNum2("" + doubleCheckMoney) + ")。\n(有問題請洽 [行銷企劃室])",
              "訊息", JOptionPane.ERROR_MESSAGE);
          jtabbedPane1.setSelectedIndex(intTablePanel);
          return false;
        }
    ...
    if ((stringFlow.indexOf("採購") != -1 || booleanApply) && booleanUndergoWriteCheck) {
      // Hashtable hashtableRealMoneyS = (Hashtable)
      // get("Doc3M010_PurchaseMoney_CostID") ; // [請購資訊] 及 [案別分攤] 金額 一致檢核
      ...
      for (int intNo = 0; intNo < vectorCostID.size(); intNo++) {
        ...
        // System.out.println(intNo+"[請購資訊] 及 [案別分攤] 金額
        // 一致檢核-----stringKey("+stringKey+")doubleRealMoneySum("+doubleRealMoneySum+")doubleRealMoneySumS("+doubleRealMoneySumS+")----------------------------------------")
        // ;
        ...
        if (doubleRealMoneySum != doubleRealMoneySumS) {
          ...
          JOptionPane.showMessageDialog(null,
              "請款代碼(" + stringCostID + ")、小請款代碼(" + stringCostID1 + ") 之 對應的 [ 案別分攤](" + doubleRealMoneySum + ") 及 [採購資訊](" + doubleRealMoneySumS + ") 總金額不相等。", "訊息",
              JOptionPane.ERROR_MESSAGE);
          return false;
        }
      }
    ...
    if (booleanCheckNew) {
      ...
      for (int intNo = 0; intNo < arrayProjectID1BudgetID.length; intNo++) {
        ...
        stringBudgetID = arrayTemp[1].trim();
        ...
        arrayTemp[0] = stringProjectID1 + "%-%" + stringBudgetID + "%-%" + doubleBudgetMoneyL + "%-%" + doubleBudgetMoneySumL + "%-%" + doubleBudgetMoneyL2 + "%-%"
            + doubleBudgetMoneySumL2;
  ...
  public Vector getFunctionTypeUDoc2M0201(FargloryUtil exeUtil, Doc2M010 exeFun) throws Throwable {
    ...
    String stringSqlAnd = " AND  (ComNo  =  '" + stringComNo + "'  OR  ComNo  =  'ALL')  AND  FunctionType  LIKE  '%U%'  AND  (DateStart  <=  '" + stringCDateAC
        + "'  OR  DateStart  = '9999/99/99')  AND  (DateEnd  >=  '" + stringCDateAC + "') ";
    String stringCostID = "";
    String stringCostID1 = "";
    ...
    for (int intNo = 0; intNo < vectorDoc2M0201.size(); intNo++) {
      hashtableDoc2M0201 = (Hashtable) vectorDoc2M0201.get(intNo);
      if (hashtableDoc2M0201 == null) continue;
  ...
  public void doMailData(String stringProjectID1, String stringType, String stringMailMessage, Hashtable hashtableUserIDMessage, Vector vectorUserID, Doc.Doc2M010 exeFun)
      throws Throwable {
    String stringEmployeeNo = "";
    String stringTemp = "";
  ...
  public String[][] getDoc3M040(String stringComNo, String stringActionNo, Doc.Doc2M010 exeFun) throws Throwable {
    ...
    //
    stringSql = "SELECT  UNDERGO_WRITE  FROM  Doc3M040  WHERE  DocNo  =  '" + stringActionNo + "'  AND  ComNo  =  '" + stringComNo + "' ";
  ...
  public Hashtable getDoc3M043(String stringActionNo, Vector vectorProject, Doc.Doc2M010 exeFun, FargloryUtil exeUtil) throws Throwable {
    ...
    //
    stringSql = "SELECT  ProjectID,  ProjectID1,  UseMoney,  DepartNo  FROM  Doc3M043  WHERE  RTRIM(DocNo1)+RTRIM(DocNo2)+RTRIM(DocNo3)  IN  (SELECT  DocNo "
        + " FROM  Doc3M040  WHERE  UNDERGO_WRITE  =  'Y'  AND  ComNo  =  '" + stringComNo + "'  AND  DocNo  =  '" + stringActionNo + "'  AND  DateStart  <=  '" + stringDateAC
  ...
  public Hashtable getUseMoneyDoc3M011(String stringBarCode, String stringActionNo, Doc.Doc2M010 exeFun, FargloryUtil exeUtil) throws Throwable {
    ...
    //
    stringSql = "SELECT  ProjectID,  ProjectID1,  (RealMoney-NoUseRealMoney),  M14.DepartNo  FROM  Doc3M014 M14,  Doc3M011 M11  WHERE  M14.BarCode  =  M11.BarCode "
        + " AND  (M11.UNDERGO_WRITE  IN  ('Y',  'C')  OR  (M11.UNDERGO_WRITE='S'  AND  ApplyType  =  'F')) AND  M11.UNDERGO_WRITE  <>  'X'  AND  M11.BarCode  <>  '" + stringBarCode
    ...
    stringSql = "SELECT  ProjectID,  ProjectID1,  (RealMoney-NoUseRealMoney),  M14.DepartNo  FROM  Doc3M014 M14,  Doc3M011 M11  WHERE  M14.BarCode  =  M11.BarCode "
        + " AND  NOT  (M11.UNDERGO_WRITE  IN  ('Y',  'C')  OR  (M11.UNDERGO_WRITE='S'  AND  ApplyType  =  'F')) AND  M11.UNDERGO_WRITE  <>  'X'  AND  M11.BarCode  <>  '"
        + stringBarCode + "'  AND  M11.ActionNo  =  '" + stringActionNo + "' ";
    retDoc3M014 = exeFun.getTableDataDoc(stringSql);
    for (int intNo = 0; intNo < retDoc3M014.length; intNo++) {
      stringProjectID = retDoc3M014[intNo][0].trim();
      stringProjectID1 = retDoc3M014[intNo][1].trim();
  ...
  public String[][] getDoc8M010(Doc.Doc2M010 exeFun) throws Throwable {
    ...
    // 0 數目 1 AcceptRealDate 2 PickRealDate
    stringSql = " SELECT  COUNT(*),   AcceptRealDate,  PickRealDate  FROM  Doc8M010  WHERE  ComNo  =  '" + convert.ToSql(stringComNo) + "'  AND  BarCode  =  '"
        + convert.ToSql(stringBarCode) + "'  AND  DocNo  =  '" + convert.ToSql(stringDocNo) + "'  GROUP BY  AcceptRealDate,  PickRealDate ";
  ...
  public boolean isTable12CheckOK(FargloryUtil exeUtil, Doc2M010 exeFun) throws Throwable {
    ...
    /*
     * if(jtable10.getRowCount()==0) {
     * System.out.println("isTable12CheckOK------------------------4") ; return true
     * ; }
     */
    // 線上簽核時，附件不可為空白
    ...
    for (int intNo = 0; intNo < jtable12.getRowCount(); intNo++) {
       ...
       * jtabbedPane1.setSelectedIndex(intTablePanel) ;
       * jtable12.setRowSelectionInterval(intNo, intNo) ;
       * messagebox("附件表格之第 "+(intNo+1)+" 行之 名稱 不可空白。") ; return false ; }
       */
      // 說明
  ...
  public boolean isTable12NoPageOK(FargloryUtil exeUtil, Doc2M010 exeFun) throws Throwable {
    ...
    String stringCostID = "";
    String stringCostID1 = "";
    String stringSqlAnd = " AND  CostIDType  =  'A'  AND  UseType  =  'A'  AND  DeptCd_Doc  LIKE  '" + exeUtil.doSubstring(stringDocNo1, 0, 3) + "%' "
        + " AND  (UseDateStart  = '9999/99/99' OR  UseDateStart<='" + stringCDate + "')  AND  (UseDateEnd  = '9999/99/99'   OR  UseDateEnd>='" + stringCDate + "') ";
  ...
  public boolean isUpdateFileOK(int intRowPos, String stringStubPath, String stringYYYYMM, Hashtable hashtableStubPath) throws Throwable {
    ...
    String stringBarCode = getValue("BarCode").trim();
    String stringBarCodeT = "" + get("BarCode_Tmp");
    put("BarCode_Tmp", "null");
    if (!"null".equals(stringBarCodeT) && "".equals(stringBarCode)) stringBarCode = stringBarCodeT;
    String stringPutDateTime = datetime.getTime("YYYY/mm/dd h:m:s");
    stringPutDateTime = stringPutDateTime.replaceAll(" ", "_").replaceAll("/", "").replaceAll(":", "");
  ...
  public void doBackStubPath(String stringFilePath, Hashtable hashtableStubPath) throws Throwable {
    ...
    for (int intNo = 0; intNo < jtable12.getRowCount(); intNo++) {
      stringStubPathOld = ("" + hashtableStubPath.get("" + intNo)).trim();
      if ("null".equals(stringStubPathOld)) continue;
      //
  ...
  public boolean isTable18CheckOK(boolean booleanUndergoWriteCheck, FargloryUtil exeUtil, Doc2M010 exeFun) throws Throwable {
    ...
    String stringNoPageDate = "" + get("NO_PAGE_DATE");
    if (stringNoPageDate.length() != 10) stringNoPageDate = "2017/12/31";// 請購無紙化上線日期
    String stringToday = exeUtil.getDateConvert(getValue("CDate"));
    String stringApplyType = getValue("ApplyType").trim();
    String stringBarCode = getValue("BarCode").trim();
    ...
    // 得標廠商之決標價須一致請購合約金額
    /*
     * double doublePurchaseMoney1 = 0 ; double doublePurchaseMoney18 = 0 ; for(int
     * intNo=0 ; intNo<vectorFactoryNo1.size() ; intNo++) { stringFactoryNo =
     * ""+vectorFactoryNo1.get(intNo) ; doublePurchaseMoney1 =
     * exeUtil.doParseDouble(convert.FourToFive(""+hashtablePurchaseMoney1.get(
     * stringFactoryNo), 0)) ; doublePurchaseMoney18 =
     * exeUtil.doParseDouble(convert.FourToFive(""+hashtablePurchaseMoney18.get(
     * stringFactoryNo), 0)) ; if(doublePurchaseMoney1 != doublePurchaseMoney18) {
     * jtabbedPane1.setSelectedIndex(intTablePanel) ;
     * messagebox("廠商議價記錄表格 與  請購項目表格 之 廠商("+stringFactoryNo+")[得標價] 不一致。") ; return
     * false ; } }
     */
    return true;
  }
